...
JASMIN_SOURCE="$REPO_ROOT/ntruplus/jasmin/768/ref/invntt_final.jazz"
TEST_DIR="$REPO_ROOT/tests/ntruplus768/invntt_final"
PROOF_DIR="$REPO_ROOT/ntruplus/proof/768/ref/invntt_final"
...
SLICE=jade_ntruplus_ntruplus768_amd64_ref_invntt_final

TMPDIR_PARENT=${TMPDIR:-/tmp}
WORKDIR=$(mktemp -d "$TMPDIR_PARENT/ntruplus768-invntt-final.XXXXXX")
WHY3_PID=
WHY3SERVER_BIN=
...
reject_proof_holes() {
  ...
  if grep -ERni --include='*.ec' \
    ...
    "$INVNTT_RADIX2_64_PROOF" "$INVNTT_RADIX2_32_PROOF" \
    "$INVNTT_RADIX2_16_PROOF" "$INVNTT_RADIX2_8_PROOF" \
    "$INVNTT_RADIX2_4_PROOF" "$INVNTT_RADIX3_PROOF" \
    >"$findings"; then
...
compare_extraction() {
  ...
  find "$TRACKED_EXTRACTED" -maxdepth 1 -type f -name '*.ec' -printf '%f\n' | sort >"$tracked_list"
  find "$generated_dir" -maxdepth 1 -type f -name '*.ec' -printf '%f\n' | sort >"$generated_list"

  if ! grep -qx 'NTRUPlus768InvNTTFinal.ec' "$tracked_list"; then
    fail "tracked inverse final extraction is missing the primary EasyCrypt file"
  fi

  generated_count=$(wc -l <"$generated_list")
  case "$generated_count" in
    1)
      grep -qx 'NTRUPlus768InvNTTFinal.ec' "$generated_list" ||
        fail "generated inverse final extraction emitted an unexpected file set"
      ;;
    3)
      cmp -s "$tracked_list" "$generated_list" ||
        fail "generated inverse final extraction emitted an unexpected file set"
      ;;
    *)
      sed -n '1,40p' "$generated_list" >&2
      fail "generated inverse final extraction emitted an unexpected number of files"
      ;;
  esac

  if ! cmp -s \
    "$generated_dir/NTRUPlus768InvNTTFinal.ec" \
    "$TRACKED_EXTRACTED/NTRUPlus768InvNTTFinal.ec"; then
    diff -u \
      "$TRACKED_EXTRACTED/NTRUPlus768InvNTTFinal.ec" \
      "$generated_dir/NTRUPlus768InvNTTFinal.ec" | sed -n '1,120p' >&2 || true
    fail "tracked inverse final EasyCrypt extraction is stale: NTRUPlus768InvNTTFinal.ec"
  fi

  for helper_clone in Array768.ec WArray1536.ec; do
    [[ -f "$TRACKED_EXTRACTED/$helper_clone" ]] ||
      fail "tracked inverse final extraction is missing helper clone: $helper_clone"
    if [[ -f "$generated_dir/$helper_clone" ]]; then
      if ! cmp -s "$generated_dir/$helper_clone" "$TRACKED_EXTRACTED/$helper_clone"; then
        diff -u "$TRACKED_EXTRACTED/$helper_clone" "$generated_dir/$helper_clone" \
          | sed -n '1,120p' >&2 || true
        fail "tracked inverse final EasyCrypt extraction is stale: $helper_clone"
      fi
      continue
    fi
    if ! cmp -s "$TRACKED_EXTRACTED/$helper_clone" "$INVNTT_RADIX3_EXTRACTED/$helper_clone"; then
      diff -u "$INVNTT_RADIX3_EXTRACTED/$helper_clone" "$TRACKED_EXTRACTED/$helper_clone" \
        | sed -n '1,120p' >&2 || true
      fail "tracked inverse final helper clone is stale: $helper_clone"
    fi
  done
}
...
main() {
  ...
  [[ -d "$TEST_DIR" ]] || fail "missing inverse final test tree: $TEST_DIR"
  [[ -f "$TEST_DIR/Makefile" ]] || fail "missing inverse final test Makefile"
  [[ -f "$TEST_DIR/check_invntt_final.py" ]] || fail "missing inverse final checker"
  [[ -f "$TEST_DIR/invntt_final_diff.c" ]] || fail "missing inverse final differential test source"
  [[ -f "$PROOF_DIR/Makefile" ]] || fail "missing inverse final proof Makefile"
  [[ -f "$PROOF_DIR/easycrypt.project" ]] || fail "missing inverse final easycrypt.project"
  [[ -f "$PROOF_DIR/NTRUPlus768InvNTTFinalProof.ec" ]] ||
    fail "missing inverse final word-level proof"
  [[ -f "$PROOF_DIR/NTRUPlus768InvNTTFinalAlgebra.ec" ]] ||
    fail "missing inverse final algebra proof"
  [[ -d "$TRACKED_EXTRACTED" ]] ||
    fail "missing tracked inverse final extraction tree: $TRACKED_EXTRACTED"
  ...
  [[ -f "$INVNTT_RADIX2_64_PROOF/NTRUPlus768InvNTTRadix2_64Algebra.ec" ]] ||
    fail "missing dependent inverse radix-2 step64 algebra proof tree"
  [[ -d "$INVNTT_RADIX2_64_EXTRACTED" ]] ||
    fail "missing dependent inverse radix-2 step64 extraction tree"
  [[ -f "$INVNTT_RADIX2_32_PROOF/NTRUPlus768InvNTTRadix2_32Algebra.ec" ]] ||
    fail "missing dependent inverse radix-2 step32 algebra proof tree"
  [[ -d "$INVNTT_RADIX2_32_EXTRACTED" ]] ||
    fail "missing dependent inverse radix-2 step32 extraction tree"
  [[ -f "$INVNTT_RADIX2_16_PROOF/NTRUPlus768InvNTTRadix2_16Algebra.ec" ]] ||
    fail "missing dependent inverse radix-2 step16 algebra proof tree"
  [[ -d "$INVNTT_RADIX2_16_EXTRACTED" ]] ||
    fail "missing dependent inverse radix-2 step16 extraction tree"
  ...
  [[ -f "$INVNTT_RADIX3_PROOF/NTRUPlus768InvNTTRadix3Algebra.ec" ]] ||
    fail "missing dependent inverse radix-3 algebra proof tree"
  [[ -d "$INVNTT_RADIX3_EXTRACTED" ]] ||
    fail "missing dependent inverse radix-3 extraction tree"
  ...
  jasmin2ec "$JASMIN_SOURCE" --array-model=old \
    -o "$generated_dir/NTRUPlus768InvNTTFinal.ec" -f "$SLICE"
  compare_extraction "$generated_dir"
  reject_proof_holes

  compile_easycrypt "$PROOF_DIR/NTRUPlus768InvNTTFinalProof.ec" word-level
  compile_easycrypt "$PROOF_DIR/NTRUPlus768InvNTTFinalAlgebra.ec" algebra

  jasminc -arch x86-64 -call-conv linux -system linux \
    -slice "$SLICE" -o "$asm_dir/invntt_final.s" "$JASMIN_SOURCE"
  jasminc -arch x86-64 -call-conv linux -system linux -checksafety \
    -slice "$SLICE" -o "$asm_dir/invntt_final-safety.s" "$JASMIN_SOURCE"
  ...
  printf 'PASS: verify-ntruplus768-invntt-final.sh syntax check\n'
  printf 'PASS: NTRU+768 inverse final C/Jasmin coupling and checker self-check\n'
  printf 'PASS: NTRU+768 inverse final Jasmin build, safety, CT, and SCT checks\n'
  printf 'PASS: NTRU+768 inverse final standalone, chained differential, and UBSan tests\n'
  printf 'PASS: NTRU+768 inverse final fresh extraction matches tracked EasyCrypt model\n'
  printf 'PASS: NTRU+768 inverse final EasyCrypt word-level functional proof\n'
  printf 'PASS: NTRU+768 inverse final recombination, scaling, and algebra bridge\n'
}
