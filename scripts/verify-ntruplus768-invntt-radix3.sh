...
JASMIN_SOURCE="$REPO_ROOT/ntruplus/jasmin/768/ref/invntt_radix3.jazz"
TEST_DIR="$REPO_ROOT/tests/ntruplus768/invntt_radix3"
PROOF_DIR="$REPO_ROOT/ntruplus/proof/768/ref/invntt_radix3"
...
WORKDIR=$(mktemp -d "$TMPDIR_PARENT/ntruplus768-invntt-radix3.XXXXXX")
WHY3_PID=
WHY3SERVER_BIN=
...
compare_extraction() {
  ...
  if ! grep -qx 'NTRUPlus768InvNTTRadix3.ec' "$tracked_list"; then
    fail "tracked inverse radix-3 extraction is missing the primary EasyCrypt file"
  fi

  generated_count=$(wc -l <"$generated_list")
  case "$generated_count" in
    1)
      grep -qx 'NTRUPlus768InvNTTRadix3.ec' "$generated_list" ||
        fail "generated inverse radix-3 extraction emitted an unexpected file set"
      ;;
    3)
    ...
    *)
      ...
      fail "generated inverse radix-3 extraction emitted an unexpected number of files"
      ;;
  esac

  if ! cmp -s \
    "$generated_dir/NTRUPlus768InvNTTRadix3.ec" \
    "$TRACKED_EXTRACTED/NTRUPlus768InvNTTRadix3.ec"; then
    diff -u \
      "$TRACKED_EXTRACTED/NTRUPlus768InvNTTRadix3.ec" \
      "$generated_dir/NTRUPlus768InvNTTRadix3.ec" | sed -n '1,120p' >&2 || true
    fail "tracked inverse radix-3 EasyCrypt extraction is stale: NTRUPlus768InvNTTRadix3.ec"
  fi

  for helper_clone in Array768.ec WArray1536.ec; do
    [[ -f "$TRACKED_EXTRACTED/$helper_clone" ]] ||
      fail "tracked inverse radix-3 extraction is missing helper clone: $helper_clone"
    if [[ -f "$generated_dir/$helper_clone" ]]; then
      if ! cmp -s "$generated_dir/$helper_clone" "$TRACKED_EXTRACTED/$helper_clone"; then
        diff -u "$TRACKED_EXTRACTED/$helper_clone" "$generated_dir/$helper_clone" \
          | sed -n '1,120p' >&2 || true
        fail "tracked inverse radix-3 EasyCrypt extraction is stale: $helper_clone"
      fi
      continue
    fi
    if ! cmp -s "$TRACKED_EXTRACTED/$helper_clone" "$INVNTT_RADIX2_64_EXTRACTED/$helper_clone"; then
      diff -u "$INVNTT_RADIX2_64_EXTRACTED/$helper_clone" "$TRACKED_EXTRACTED/$helper_clone" \
        | sed -n '1,120p' >&2 || true
      fail "tracked inverse radix-3 helper clone is stale: $helper_clone"
    fi
  done
}
...
main() {
  ...
  [[ -d "$TEST_DIR" ]] || fail "missing inverse radix-3 test tree: $TEST_DIR"
  [[ -f "$PROOF_DIR/NTRUPlus768InvNTTRadix3Proof.ec" ]] ||
    fail "missing inverse radix-3 word-level proof"
  [[ -f "$PROOF_DIR/NTRUPlus768InvNTTRadix3Algebra.ec" ]] ||
    fail "missing inverse radix-3 algebra proof"
  [[ -d "$TRACKED_EXTRACTED" ]] ||
    fail "missing tracked inverse radix-3 extraction tree: $TRACKED_EXTRACTED"
  ...
  jasmin2ec "$JASMIN_SOURCE" --array-model=old \
    -o "$generated_dir/NTRUPlus768InvNTTRadix3.ec" -f "$SLICE"
  compare_extraction "$generated_dir"
  reject_proof_holes

  compile_easycrypt "$PROOF_DIR/NTRUPlus768InvNTTRadix3Proof.ec" word-level
  compile_easycrypt "$PROOF_DIR/NTRUPlus768InvNTTRadix3Algebra.ec" algebra

  jasminc -arch x86-64 -call-conv linux -system linux \
    -slice "$SLICE" -o "$asm_dir/invntt_radix3.s" "$JASMIN_SOURCE"
  jasminc -arch x86-64 -call-conv linux -system linux -checksafety \
    -slice "$SLICE" -o "$asm_dir/invntt_radix3-safety.s" "$JASMIN_SOURCE"
  ...
  printf 'PASS: verify-ntruplus768-invntt-radix3.sh syntax check\n'
  printf 'PASS: NTRU+768 inverse radix-3 C/Jasmin coupling and checker self-check\n'
  printf 'PASS: NTRU+768 inverse radix-3 Jasmin build, safety, CT, and SCT checks\n'
  printf 'PASS: NTRU+768 inverse radix-3 standalone, composed differential, and UBSan tests\n'
  printf 'PASS: NTRU+768 inverse radix-3 fresh extraction matches tracked EasyCrypt model\n'
  printf 'PASS: NTRU+768 inverse radix-3 EasyCrypt word-level functional proof\n'
  printf 'PASS: NTRU+768 inverse radix-3 reverse-schedule, range, and algebra bridge\n'
}
