...
PROOF_FILE="$PROOF_DIR/NTRUPlus768InverseNTTRadix2_8Semantics.ec"
RECOMBINATION_FILE="$PROOF_DIR/NTRUPlus768InverseNTTRadix2_8Recombination.ec"
PREVIOUS_PROOF_FILE="$PROOF_DIR/NTRUPlus768InverseNTTRadix2_4Semantics.ec"
PREVIOUS_RECOMBINATION_FILE="$PROOF_DIR/NTRUPlus768InverseNTTRadix2_4Recombination.ec"
...
WORKDIR=$(mktemp -d "$TMPDIR_PARENT/ntruplus768-invntt-radix2-8-semantics.XXXXXX")
...
reject_proof_holes() {
  ...
  if grep -ERni --include='*.ec' \
    ...
    fail "inverse-NTT radix2_8 semantics proof tree contains a proof-hole keyword"
  else
    status=$?
...
compile_easycrypt_via_makefile() {
  ...
  if ! make -C "$PROOF_DIR" inverse-radix2-8 EASYCRYPT="$easycrypt_bin"; then
    fail "EasyCrypt inverse-NTT radix2_8 semantics proof failed"
  fi
}
...
main() {
  ...
  bash -n "$0"
  require_lemma invntt_radix2_8_schedule_forwardE
  require_lemma invntt_radix2_8_child_scheduleE
  require_lemma invntt_radix2_8_schedule_sumE
  require_lemma invntt_radix2_8_schedule_nonneg
  require_lemma invntt_radix2_8_child_exp_nonneg
  require_lemma invntt_radix2_8_root_cancelE
  require_lemma invntt_radix2_8_segment_leftE
  require_operation invntt_radix2_8_scaled_diff_poly
  require_lemma invntt_radix2_8_scaled_diff_polyE
  require_lemma segment_poly_scaled_diff2_8E
  require_lemma invntt_radix2_8_segment_rightE
  require_lemma invntt_radix2_8_output_segmentE
  require_lemma invntt_radix2_8_recombine_factor_eqm
  require_operation invntt_radix2_8_semantics
  require_lemma invntt_radix2_8_algebra_refines_invntt_radix2_4_semantics
  require_lemma invntt_radix2_8_spec_semantics
  require_lemma invntt_radix2_8_semantics_functional
  require_lemma invntt_radix2_8_correct_semantics
  reject_proof_holes

  "$REPO_ROOT/scripts/verify-ntruplus768-invntt-radix2-4-semantics.sh"
  "$REPO_ROOT/scripts/verify-ntruplus768-invntt-radix2-8.sh"
  compile_easycrypt_via_makefile

  printf 'PASS: inverse step8 pairs the two scheduled degree-8 child factors\n'
  printf 'PASS: each inverse radix2_8 block reconstructs one degree-16 residue of 4p\n'
  printf 'PASS: executable Jasmin inverse radix2_8 extends the inverse ring-semantics chain\n'
  printf 'PASS: EasyCrypt NTRU+768 inverse radix2_8 polynomial semantics\n'
}
