...
JASMIN_SOURCE="$REPO_ROOT/ntruplus/jasmin/768/ref/ntt_radix3.jazz"
TEST_DIR="$REPO_ROOT/tests/ntruplus768/ntt_radix3"
PROOF_DIR="$REPO_ROOT/ntruplus/proof/768/ref/ntt_radix3"
...
SLICE=jade_ntruplus_ntruplus768_amd64_ref_ntt_radix3

TMPDIR_PARENT=${TMPDIR:-/tmp}
WORKDIR=$(mktemp -d "$TMPDIR_PARENT/ntruplus768-ntt-radix3.XXXXXX")
WHY3_PID=
WHY3SERVER_BIN=
...
reject_proof_holes() {
  ...
  if grep -ERni --include='*.ec' \
    ...
    "$PROOF_DIR" "$STAGE1_PROOF" "$BASEMUL_PROOF" "$NTT_SCHEDULE_PROOF" >"$findings"; then
...
compare_extraction() {
  ...
  if ! cmp -s "$expected_list" "$generated_list"; then
    ...
    fail "generated extraction file set does not match expected NTT radix-3 extraction"
  fi
  ...
  while IFS= read -r name; do
    if ! cmp -s "$generated_dir/$name" "$TRACKED_EXTRACTED/$name"; then
      ...
      fail "tracked NTT radix-3 EasyCrypt extraction is stale: $name"
    fi
  done <"$expected_list"
}
...
main() {
  ...
  [[ -f "$PROOF_DIR/NTRUPlus768NTTRadix3Proof.ec" ]] ||
    fail "missing NTT radix-3 word-level proof"
  [[ -f "$PROOF_DIR/NTRUPlus768NTTRadix3Algebra.ec" ]] ||
    fail "missing NTT radix-3 algebra proof"
  ...
  [[ -f "$STAGE1_PROOF/NTRUPlus768NTTStage1Algebra.ec" ]] ||
    fail "missing dependent NTT stage-1 proof tree"
  ...
  jasmin2ec "$JASMIN_SOURCE" --array-model=old \
    -o "$generated_dir/NTRUPlus768NTTRadix3.ec" -f "$SLICE"
  compare_extraction "$generated_dir"
  reject_proof_holes

  compile_easycrypt "$PROOF_DIR/NTRUPlus768NTTRadix3Proof.ec" word-level
  compile_easycrypt "$PROOF_DIR/NTRUPlus768NTTRadix3Algebra.ec" algebra

  jasminc -arch x86-64 -call-conv linux -system linux \
    -slice "$SLICE" -o "$asm_dir/ntt_radix3.s" "$JASMIN_SOURCE"
  jasminc -arch x86-64 -call-conv linux -system linux -checksafety \
    -slice "$SLICE" -o "$asm_dir/ntt_radix3-safety.s" "$JASMIN_SOURCE"
  ...
  printf 'PASS: verify-ntruplus768-ntt-radix3.sh syntax check\n'
  printf 'PASS: NTRU+768 radix-3 C/Jasmin source coupling and checker self-check\n'
  printf 'PASS: NTRU+768 radix-3 Jasmin build, safety, CT, and SCT checks\n'
  printf 'PASS: NTRU+768 radix-3 differential, chained stage-1, and UBSan tests\n'
  printf 'PASS: NTRU+768 radix-3 fresh extraction matches tracked EasyCrypt model\n'
  printf 'PASS: NTRU+768 radix-3 EasyCrypt word-level functional proof\n'
  printf 'PASS: NTRU+768 radix-3 EasyCrypt root-schedule, range, and algebra bridge\n'
}
