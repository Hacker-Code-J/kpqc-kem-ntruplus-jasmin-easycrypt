...
JASMIN_SOURCE="$REPO_ROOT/ntruplus/jasmin/768/ref/invntt_radix2_32.jazz"
TEST_DIR="$REPO_ROOT/tests/ntruplus768/invntt_radix2_32"
PROOF_DIR="$REPO_ROOT/ntruplus/proof/768/ref/invntt_radix2_32"
...
SLICE=jade_ntruplus_ntruplus768_amd64_ref_invntt_radix2_32

TMPDIR_PARENT=${TMPDIR:-/tmp}
WORKDIR=$(mktemp -d "$TMPDIR_PARENT/ntruplus768-invntt-radix2-32.XXXXXX")
WHY3_PID=
WHY3SERVER_BIN=
...
reject_proof_holes() {
  ...
  if grep -ERni --include='*.ec' \
    ...
    "$INVNTT_RADIX2_16_PROOF" "$INVNTT_RADIX2_16_EXTRACTED" \
...
compare_extraction() {
  ...
  if ! cmp -s "$expected_list" "$generated_list"; then
    ...
    fail "generated extraction file set does not match expected inverse radix-2 step32 extraction"
  fi
  ...
  while IFS= read -r name; do
    if ! cmp -s "$generated_dir/$name" "$TRACKED_EXTRACTED/$name"; then
      ...
      fail "tracked inverse radix-2 step32 EasyCrypt extraction is stale: $name"
    fi
  done <"$expected_list"
}
...
main() {
  ...
  [[ -d "$TEST_DIR" ]] || fail "missing inverse radix-2 step32 test tree: $TEST_DIR"
  [[ -f "$PROOF_DIR/NTRUPlus768InvNTTRadix2_32Proof.ec" ]] ||
    fail "missing inverse radix-2 step32 word-level proof"
  [[ -f "$PROOF_DIR/NTRUPlus768InvNTTRadix2_32Algebra.ec" ]] ||
    fail "missing inverse radix-2 step32 algebra proof"
  [[ -d "$TRACKED_EXTRACTED" ]] ||
    fail "missing tracked inverse radix-2 step32 extraction tree: $TRACKED_EXTRACTED"
  ...
  jasmin2ec "$JASMIN_SOURCE" --array-model=old \
    -o "$generated_dir/NTRUPlus768InvNTTRadix2_32.ec" -f "$SLICE"
  compare_extraction "$generated_dir"
  reject_proof_holes

  compile_easycrypt "$PROOF_DIR/NTRUPlus768InvNTTRadix2_32Proof.ec" word-level
  compile_easycrypt "$PROOF_DIR/NTRUPlus768InvNTTRadix2_32Algebra.ec" algebra

  jasminc -arch x86-64 -call-conv linux -system linux \
    -slice "$SLICE" -o "$asm_dir/invntt_radix2_32.s" "$JASMIN_SOURCE"
  jasminc -arch x86-64 -call-conv linux -system linux -checksafety \
    -slice "$SLICE" -o "$asm_dir/invntt_radix2_32-safety.s" "$JASMIN_SOURCE"
  ...
  printf 'PASS: verify-ntruplus768-invntt-radix2-32.sh syntax check\n'
  printf 'PASS: NTRU+768 inverse radix-2 step32 C/Jasmin coupling and checker self-check\n'
  printf 'PASS: NTRU+768 inverse radix-2 step32 Jasmin build, safety, CT, and SCT checks\n'
  printf 'PASS: NTRU+768 inverse radix-2 step32 differential and UBSan tests\n'
  printf 'PASS: NTRU+768 inverse radix-2 step32 fresh extraction matches tracked EasyCrypt model\n'
  printf 'PASS: NTRU+768 inverse radix-2 step32 EasyCrypt word-level functional proof\n'
  printf 'PASS: NTRU+768 inverse radix-2 step32 reverse-schedule, range, and algebra bridge\n'
}
