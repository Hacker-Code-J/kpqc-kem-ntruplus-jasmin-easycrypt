...
TEST_DIR="$REPO_ROOT/tests/ntruplus768/ntt"
PROOF_DIR="$REPO_ROOT/ntruplus/proof/768/ref/ntt"
TRACKED_EXTRACTED="$PROOF_DIR/extracted"
STAGE1_PROOF="$REPO_ROOT/ntruplus/proof/768/ref/ntt_stage1"
STAGE1_EXTRACTED="$STAGE1_PROOF/extracted"
...
RADIX2_16_PROOF="$REPO_ROOT/ntruplus/proof/768/ref/ntt_radix2_16"
RADIX2_16_EXTRACTED="$RADIX2_16_PROOF/extracted"
RADIX2_8_PROOF="$REPO_ROOT/ntruplus/proof/768/ref/ntt_radix2_8"
RADIX2_8_EXTRACTED="$RADIX2_8_PROOF/extracted"
RADIX2_4_PROOF="$REPO_ROOT/ntruplus/proof/768/ref/ntt_radix2_4"
RADIX2_4_EXTRACTED="$RADIX2_4_PROOF/extracted"
...
SLICE=jade_ntruplus_ntruplus768_amd64_ref_ntt

TMPDIR_PARENT=${TMPDIR:-/tmp}
WORKDIR=$(mktemp -d "$TMPDIR_PARENT/ntruplus768-ntt.XXXXXX")
WHY3_PID=
WHY3SERVER_BIN=
...
reject_proof_holes() {
  ...
  if grep -ERni --include='*.ec' \
    ...
    "$PROOF_DIR" "$STAGE1_PROOF" "$RADIX3_PROOF" "$RADIX2_64_PROOF" \
    "$RADIX2_32_PROOF" "$RADIX2_16_PROOF" "$RADIX2_8_PROOF" "$RADIX2_4_PROOF" \
...
compile_easycrypt() {
  ...
  if ! easycrypt compile -no-eco -p Z3 -timeout 5 -max-provers 1 \
    ...
    -I "$RADIX2_16_PROOF" \
    -I "$RADIX2_16_EXTRACTED" \
    -I "$RADIX2_8_PROOF" \
    -I "$RADIX2_8_EXTRACTED" \
    -I "$RADIX2_4_PROOF" \
    -I "$RADIX2_4_EXTRACTED" \
    -I "$BASEMUL_PROOF" \
    -I "$BASEMUL_EXTRACTED" \
...
compare_extraction() {
  ...
  if ! cmp -s "$expected_list" "$generated_list"; then
    ...
    fail "generated extraction file set does not match expected composed NTT extraction"
  fi
  ...
  while IFS= read -r name; do
    if ! cmp -s "$generated_dir/$name" "$TRACKED_EXTRACTED/$name"; then
      ...
      fail "tracked composed NTT EasyCrypt extraction is stale: $name"
    fi
  done <"$expected_list"
}
...
main() {
  ...
  [[ -d "$TEST_DIR" ]] || fail "missing composed NTT test tree: $TEST_DIR"
  [[ -f "$PROOF_DIR/Makefile" ]] || fail "missing composed NTT proof Makefile"
  [[ -f "$PROOF_DIR/easycrypt.project" ]] || fail "missing composed NTT easycrypt.project"
  [[ -f "$PROOF_DIR/NTRUPlus768NTTProof.ec" ]] ||
    fail "missing composed NTT word-level proof"
  [[ -f "$PROOF_DIR/NTRUPlus768NTTAlgebra.ec" ]] ||
    fail "missing composed NTT algebra proof"
  [[ -d "$TRACKED_EXTRACTED" ]] ||
    fail "missing tracked composed NTT extraction tree: $TRACKED_EXTRACTED"
  ...
  [[ -f "$STAGE1_PROOF/NTRUPlus768NTTStage1Algebra.ec" ]] ||
    fail "missing dependent NTT stage1 proof tree"
  [[ -d "$STAGE1_EXTRACTED" ]] ||
    fail "missing dependent NTT stage1 extraction tree"
  ...
  [[ -f "$RADIX2_16_PROOF/NTRUPlus768NTTRadix2_16Algebra.ec" ]] ||
    fail "missing dependent NTT radix-2 step16 proof tree"
  [[ -d "$RADIX2_16_EXTRACTED" ]] ||
    fail "missing dependent NTT radix-2 step16 extraction tree"
  [[ -f "$RADIX2_8_PROOF/NTRUPlus768NTTRadix2_8Algebra.ec" ]] ||
    fail "missing dependent NTT radix-2 step8 proof tree"
  [[ -d "$RADIX2_8_EXTRACTED" ]] ||
    fail "missing dependent NTT radix-2 step8 extraction tree"
  [[ -f "$RADIX2_4_PROOF/NTRUPlus768NTTRadix2_4Algebra.ec" ]] ||
    fail "missing dependent NTT radix-2 step4 proof tree"
  [[ -d "$RADIX2_4_EXTRACTED" ]] ||
    fail "missing dependent NTT radix-2 step4 extraction tree"
  ...
  jasmin2ec "$JASMIN_SOURCE" --array-model=old \
    -o "$generated_dir/NTRUPlus768NTT.ec" -f "$SLICE"
  compare_extraction "$generated_dir"
  reject_proof_holes

  compile_easycrypt "$PROOF_DIR/NTRUPlus768NTTProof.ec" word-level
  compile_easycrypt "$PROOF_DIR/NTRUPlus768NTTAlgebra.ec" algebra

  jasminc -arch x86-64 -call-conv linux -system linux \
    -slice "$SLICE" -o "$asm_dir/ntt.s" "$JASMIN_SOURCE"
  jasminc -arch x86-64 -call-conv linux -system linux -checksafety \
    -slice "$SLICE" -o "$asm_dir/ntt-safety.s" "$JASMIN_SOURCE"
  ...
  printf 'PASS: verify-ntruplus768-ntt.sh syntax check\n'
  printf 'PASS: NTRU+768 composed forward-NTT C/Jasmin source coupling and checker self-check\n'
  printf 'PASS: NTRU+768 composed forward-NTT Jasmin build, safety, CT, and SCT checks\n'
  printf 'PASS: NTRU+768 composed forward-NTT differential and UBSan tests\n'
  printf 'PASS: NTRU+768 composed forward-NTT fresh extraction matches tracked EasyCrypt model\n'
  printf 'PASS: NTRU+768 composed forward-NTT EasyCrypt word-level functional proof\n'
  printf 'PASS: NTRU+768 composed forward-NTT EasyCrypt algebra chain\n'
}
