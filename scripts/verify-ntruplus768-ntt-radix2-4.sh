...
JASMIN_SOURCE="$REPO_ROOT/ntruplus/jasmin/768/ref/ntt_radix2_4.jazz"
TEST_DIR="$REPO_ROOT/tests/ntruplus768/ntt_radix2_4"
PROOF_DIR="$REPO_ROOT/ntruplus/proof/768/ref/ntt_radix2_4"
...
SLICE=jade_ntruplus_ntruplus768_amd64_ref_ntt_radix2_4

TMPDIR_PARENT=${TMPDIR:-/tmp}
WORKDIR=$(mktemp -d "$TMPDIR_PARENT/ntruplus768-ntt-radix2-4.XXXXXX")
WHY3_PID=
WHY3SERVER_BIN=
...
reject_proof_holes() {
  ...
  if grep -ERni --include='*.ec' \
    ...
    "$PROOF_DIR" "$RADIX2_8_PROOF" "$RADIX2_16_PROOF" "$RADIX2_32_PROOF" \
    "$RADIX2_64_PROOF" "$RADIX3_PROOF" "$BASEMUL_PROOF" "$NTT_SCHEDULE_PROOF" \
    >"$findings"; then
...
compare_extraction() {
  ...
  if ! cmp -s "$expected_list" "$generated_list"; then
    ...
    fail "generated extraction file set does not match expected NTT radix-2 step4 extraction"
  fi
  ...
  while IFS= read -r name; do
    if ! cmp -s "$generated_dir/$name" "$TRACKED_EXTRACTED/$name"; then
      ...
      fail "tracked NTT radix-2 step4 EasyCrypt extraction is stale: $name"
    fi
  done <"$expected_list"
}
...
main() {
  ...
  [[ -d "$TEST_DIR" ]] || fail "missing NTT radix-2 step4 test tree: $TEST_DIR"
  [[ -f "$PROOF_DIR/NTRUPlus768NTTRadix2_4Proof.ec" ]] ||
    fail "missing NTT radix-2 step4 word-level proof"
  [[ -f "$PROOF_DIR/NTRUPlus768NTTRadix2_4Algebra.ec" ]] ||
    fail "missing NTT radix-2 step4 algebra proof"
  [[ -d "$TRACKED_EXTRACTED" ]] ||
    fail "missing tracked NTT radix-2 step4 extraction tree: $TRACKED_EXTRACTED"
  ...
  jasmin2ec "$JASMIN_SOURCE" --array-model=old \
    -o "$generated_dir/NTRUPlus768NTTRadix2_4.ec" -f "$SLICE"
  compare_extraction "$generated_dir"
  reject_proof_holes

  compile_easycrypt "$PROOF_DIR/NTRUPlus768NTTRadix2_4Proof.ec" word-level
  compile_easycrypt "$PROOF_DIR/NTRUPlus768NTTRadix2_4Algebra.ec" algebra

  jasminc -arch x86-64 -call-conv linux -system linux \
    -slice "$SLICE" -o "$asm_dir/ntt_radix2_4.s" "$JASMIN_SOURCE"
  jasminc -arch x86-64 -call-conv linux -system linux -checksafety \
    -slice "$SLICE" -o "$asm_dir/ntt_radix2_4-safety.s" "$JASMIN_SOURCE"
  ...
  printf 'PASS: verify-ntruplus768-ntt-radix2-4.sh syntax check\n'
  printf 'PASS: NTRU+768 radix-2 step4 C/Jasmin source coupling and checker self-check\n'
  printf 'PASS: NTRU+768 radix-2 step4 Jasmin build, safety, CT, and SCT checks\n'
  printf 'PASS: NTRU+768 radix-2 step4 differential, chained-prefix, and UBSan tests\n'
  printf 'PASS: NTRU+768 radix-2 step4 fresh extraction matches tracked EasyCrypt model\n'
  printf 'PASS: NTRU+768 radix-2 step4 EasyCrypt word-level functional proof\n'
  printf 'PASS: NTRU+768 radix-2 step4 EasyCrypt schedule, centered-range, and algebra bridge\n'
}
