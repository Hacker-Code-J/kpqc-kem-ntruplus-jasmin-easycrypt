...
JASMIN_SOURCE="$REPO_ROOT/ntruplus/jasmin/768/ref/ntt_stage1.jazz"
TEST_DIR="$REPO_ROOT/tests/ntruplus768/ntt_stage1"
PROOF_DIR="$REPO_ROOT/ntruplus/proof/768/ref/ntt_stage1"
...
SLICE=jade_ntruplus_ntruplus768_amd64_ref_ntt_stage1

TMPDIR_PARENT=${TMPDIR:-/tmp}
WORKDIR=$(mktemp -d "$TMPDIR_PARENT/ntruplus768-ntt-stage1.XXXXXX")
WHY3_PID=
WHY3SERVER_BIN=
...
compare_extraction() {
  ...
  if ! cmp -s "$expected_list" "$generated_list"; then
    ...
    fail "generated extraction file set does not match expected NTT stage-1 extraction"
  fi
  ...
  while IFS= read -r name; do
    if ! cmp -s "$generated_dir/$name" "$TRACKED_EXTRACTED/$name"; then
      ...
      fail "tracked NTT stage-1 EasyCrypt extraction is stale: $name"
    fi
  done <"$expected_list"
}
...
main() {
  ...
  jasmin2ec "$JASMIN_SOURCE" --array-model=old \
    -o "$generated_dir/NTRUPlus768NTTStage1.ec" -f "$SLICE"
  compare_extraction "$generated_dir"
  reject_proof_holes

  compile_easycrypt "$PROOF_DIR/NTRUPlus768NTTStage1Proof.ec" word-level
  compile_easycrypt "$PROOF_DIR/NTRUPlus768NTTStage1Algebra.ec" algebra

  jasminc -arch x86-64 -call-conv linux -system linux \
    -slice "$SLICE" -o "$asm_dir/ntt_stage1.s" "$JASMIN_SOURCE"
  jasminc -arch x86-64 -call-conv linux -system linux -checksafety \
    -slice "$SLICE" -o "$asm_dir/ntt_stage1-safety.s" "$JASMIN_SOURCE"
  ...
  printf 'PASS: verify-ntruplus768-ntt-stage1.sh syntax check\n'
  printf 'PASS: NTRU+768 initial NTT split C/Jasmin source coupling and checker self-check\n'
  printf 'PASS: NTRU+768 initial NTT split Jasmin build, safety, CT, and SCT checks\n'
  printf 'PASS: NTRU+768 initial NTT split differential and UBSan tests\n'
  printf 'PASS: NTRU+768 initial NTT split fresh extraction matches tracked EasyCrypt model\n'
  printf 'PASS: NTRU+768 initial NTT split EasyCrypt word-level functional proof\n'
  printf 'PASS: NTRU+768 initial NTT split EasyCrypt root-schedule and range bridge\n'
}
