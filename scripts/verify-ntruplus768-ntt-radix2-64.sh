...
JASMIN_SOURCE="$REPO_ROOT/ntruplus/jasmin/768/ref/ntt_radix2_64.jazz"
TEST_DIR="$REPO_ROOT/tests/ntruplus768/ntt_radix2_64"
PROOF_DIR="$REPO_ROOT/ntruplus/proof/768/ref/ntt_radix2_64"
...
SLICE=jade_ntruplus_ntruplus768_amd64_ref_ntt_radix2_64

TMPDIR_PARENT=${TMPDIR:-/tmp}
WORKDIR=$(mktemp -d "$TMPDIR_PARENT/ntruplus768-ntt-radix2-64.XXXXXX")
WHY3_PID=
WHY3SERVER_BIN=
...
reject_proof_holes() {
  ...
  if grep -ERni --include='*.ec' \
    ...
    "$PROOF_DIR" "$RADIX3_PROOF" "$BASEMUL_PROOF" "$NTT_SCHEDULE_PROOF" >"$findings"; then
...
compare_extraction() {
  ...
  if ! cmp -s "$expected_list" "$generated_list"; then
    ...
    fail "generated extraction file set does not match expected NTT radix-2 step64 extraction"
  fi
  ...
  while IFS= read -r name; do
    if ! cmp -s "$generated_dir/$name" "$TRACKED_EXTRACTED/$name"; then
      ...
      fail "tracked NTT radix-2 step64 EasyCrypt extraction is stale: $name"
    fi
  done <"$expected_list"
}
...
main() {
  ...
  [[ -f "$PROOF_DIR/NTRUPlus768NTTRadix2_64Proof.ec" ]] ||
    fail "missing NTT radix-2 step64 word-level proof"
  [[ -f "$PROOF_DIR/NTRUPlus768NTTRadix2_64Algebra.ec" ]] ||
    fail "missing NTT radix-2 step64 algebra proof"
  ...
  jasmin2ec "$JASMIN_SOURCE" --array-model=old \
    -o "$generated_dir/NTRUPlus768NTTRadix2_64.ec" -f "$SLICE"
  compare_extraction "$generated_dir"
  reject_proof_holes

  compile_easycrypt "$PROOF_DIR/NTRUPlus768NTTRadix2_64Proof.ec" word-level
  compile_easycrypt "$PROOF_DIR/NTRUPlus768NTTRadix2_64Algebra.ec" algebra

  jasminc -arch x86-64 -call-conv linux -system linux \
    -slice "$SLICE" -o "$asm_dir/ntt_radix2_64.s" "$JASMIN_SOURCE"
  jasminc -arch x86-64 -call-conv linux -system linux -checksafety \
    -slice "$SLICE" -o "$asm_dir/ntt_radix2_64-safety.s" "$JASMIN_SOURCE"
  ...
  printf 'PASS: verify-ntruplus768-ntt-radix2-64.sh syntax check\n'
  printf 'PASS: NTRU+768 radix-2 step64 C/Jasmin source coupling and checker self-check\n'
  printf 'PASS: NTRU+768 radix-2 step64 Jasmin build, safety, CT, and SCT checks\n'
  printf 'PASS: NTRU+768 radix-2 step64 differential, chained-prefix, and UBSan tests\n'
  printf 'PASS: NTRU+768 radix-2 step64 fresh extraction matches tracked EasyCrypt model\n'
  printf 'PASS: NTRU+768 radix-2 step64 EasyCrypt word-level functional proof\n'
  printf 'PASS: NTRU+768 radix-2 step64 EasyCrypt schedule, centered-range, and algebra bridge\n'
}
