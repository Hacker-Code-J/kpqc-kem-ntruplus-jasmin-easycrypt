...
WORKDIR=$(mktemp -d "$TMPDIR_PARENT/ntruplus768-ntt-invntt-roundtrip.XXXXXX")
...
require_lemma() {
  ...
  grep -Eq "^lemma[[:space:]]+$lemma([[:space:](]|$)" \
    "$INVERSE_PROOF_FILE" "$ROUNDTRIP_PROOF_FILE" ||
...
reject_proof_holes() {
  ...
  if grep -ERni --include='*.ec' \
    ...
    fail "NTT/InvNTT round-trip semantics proof tree contains a proof-hole keyword"
  else
    status=$?
...
compile_easycrypt_via_makefile() {
  ...
  if ! make -C "$PROOF_DIR" ntt-invntt-roundtrip EASYCRYPT="$easycrypt_bin"; then
    fail "EasyCrypt NTT/InvNTT round-trip semantics proof failed"
  fi
}
...
main() {
  ...
  [[ -f "$INVERSE_PROOF_FILE" ]] ||
    fail "missing EasyCrypt proof: $INVERSE_PROOF_FILE"
  [[ -f "$ROUNDTRIP_PROOF_FILE" ]] ||
    fail "missing EasyCrypt proof: $ROUNDTRIP_PROOF_FILE"
  [[ -f "$PROOF_DIR/Makefile" ]] || fail "missing proof Makefile"

  rm -f "${CACHE_FILES[@]}"

  bash -n "$0"
  require_operation invntt_semantics
  require_operation ntt_invntt_roundtrip_semantics
  require_lemma inverse_invntt_algebra_refines_terminal_representation
  require_lemma inverse_invntt_spec_semantics
  require_lemma invntt_semantics_functional
  require_lemma invntt_correct_semantics
  require_lemma forward_ntt_spec_invntt_input_shape
  require_lemma ntt_invntt_spec_roundtrip
  reject_proof_holes

  # Wrapper implementation checks remain separate top-level commands. This
  # target rebuilds only the complete polynomial-semantics dependency chain.
  compile_easycrypt_via_makefile

  printf 'PASS: exported inverse NTT wrapper refines terminal representation to global congruence\n'
  printf 'PASS: the wrapper copy and all seven inverse stages are covered by the semantic endpoint\n'
  printf 'PASS: forward NTT output establishes both terminal representation and inverse input range\n'
  printf 'PASS: InvNTT(NTT(a)) recovers a modulo X^768-X^384+1\n'
  printf 'PASS: EasyCrypt NTRU+768 NTT/InvNTT round-trip polynomial semantics\n'
}
