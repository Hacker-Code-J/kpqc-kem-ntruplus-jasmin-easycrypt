...
JASMIN_SOURCE="$REPO_ROOT/ntruplus/jasmin/768/ref/invntt_radix2_4.jazz"
TEST_DIR="$REPO_ROOT/tests/ntruplus768/invntt_radix2_4"
PROOF_DIR="$REPO_ROOT/ntruplus/proof/768/ref/invntt_radix2_4"
...
SLICE=jade_ntruplus_ntruplus768_amd64_ref_invntt_radix2_4

TMPDIR_PARENT=${TMPDIR:-/tmp}
WORKDIR=$(mktemp -d "$TMPDIR_PARENT/ntruplus768-invntt-radix2-4.XXXXXX")
WHY3_PID=
WHY3SERVER_BIN=
...
compare_extraction() {
  ...
  if ! cmp -s "$expected_list" "$generated_list"; then
    ...
    fail "generated extraction file set does not match expected inverse radix-2 step4 extraction"
  fi
  ...
  while IFS= read -r name; do
    if ! cmp -s "$generated_dir/$name" "$TRACKED_EXTRACTED/$name"; then
      ...
      fail "tracked inverse radix-2 step4 EasyCrypt extraction is stale: $name"
    fi
  done <"$expected_list"
}
...
main() {
  ...
  [[ -d "$TEST_DIR" ]] || fail "missing inverse radix-2 step4 test tree: $TEST_DIR"
  [[ -f "$PROOF_DIR/NTRUPlus768InvNTTRadix2_4Proof.ec" ]] ||
    fail "missing inverse radix-2 step4 word-level proof"
  [[ -f "$PROOF_DIR/NTRUPlus768InvNTTRadix2_4Algebra.ec" ]] ||
    fail "missing inverse radix-2 step4 algebra proof"
  [[ -d "$TRACKED_EXTRACTED" ]] ||
    fail "missing tracked inverse radix-2 step4 extraction tree: $TRACKED_EXTRACTED"
  ...
  jasmin2ec "$JASMIN_SOURCE" --array-model=old \
    -o "$generated_dir/NTRUPlus768InvNTTRadix2_4.ec" -f "$SLICE"
  compare_extraction "$generated_dir"
  reject_proof_holes

  compile_easycrypt "$PROOF_DIR/NTRUPlus768InvNTTRadix2_4Proof.ec" word-level
  compile_easycrypt "$PROOF_DIR/NTRUPlus768InvNTTRadix2_4Algebra.ec" algebra

  jasminc -arch x86-64 -call-conv linux -system linux \
    -slice "$SLICE" -o "$asm_dir/invntt_radix2_4.s" "$JASMIN_SOURCE"
  jasminc -arch x86-64 -call-conv linux -system linux -checksafety \
    -slice "$SLICE" -o "$asm_dir/invntt_radix2_4-safety.s" "$JASMIN_SOURCE"
  ...
  printf 'PASS: verify-ntruplus768-invntt-radix2-4.sh syntax check\n'
  printf 'PASS: NTRU+768 inverse radix-2 step4 C/Jasmin coupling and checker self-check\n'
  printf 'PASS: NTRU+768 inverse radix-2 step4 Jasmin build, safety, CT, and SCT checks\n'
  printf 'PASS: NTRU+768 inverse radix-2 step4 differential and UBSan tests\n'
  printf 'PASS: NTRU+768 inverse radix-2 step4 fresh extraction matches tracked EasyCrypt model\n'
  printf 'PASS: NTRU+768 inverse radix-2 step4 EasyCrypt word-level functional proof\n'
  printf 'PASS: NTRU+768 inverse radix-2 step4 reverse-schedule, range, and algebra bridge\n'
}
