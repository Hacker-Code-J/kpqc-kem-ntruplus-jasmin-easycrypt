...
PROOF_FILE="$PROOF_DIR/NTRUPlus768InverseNTTRadix2_32Semantics.ec"
RECOMBINATION_FILE="$PROOF_DIR/NTRUPlus768InverseNTTRadix2_32Recombination.ec"
...
STEP32_PROOF_CACHE="${PROOF_FILE%.ec}.eco"
STEP32_RECOMBINATION_CACHE="${RECOMBINATION_FILE%.ec}.eco"
...
WORKDIR=$(mktemp -d "$TMPDIR_PARENT/ntruplus768-invntt-radix2-32-semantics.XXXXXX")

cleanup() {
  rm -f "$STEP32_PROOF_CACHE" "$STEP32_RECOMBINATION_CACHE" \
...
reject_proof_holes() {
  ...
  if grep -ERni --include='*.ec' \
    ...
    fail "inverse-NTT radix2_32 semantics proof tree contains a proof-hole keyword"
  else
    status=$?
...
compile_easycrypt_via_makefile() {
  ...
  if ! make -C "$PROOF_DIR" inverse-radix2-32 EASYCRYPT="$easycrypt_bin"; then
    fail "EasyCrypt inverse-NTT radix2_32 semantics proof failed"
  fi
}
...
main() {
  ...
  [[ -f "$STEP16_PROOF_FILE" ]] ||
    fail "missing predecessor EasyCrypt proof: $STEP16_PROOF_FILE"
  ...
  bash -n "$0"
  require_operation radix2_32_schedule_index
  require_operation radix2_32_schedule_exp
  require_lemma invntt_radix2_32_schedule_forwardE
  require_operation radix2_32_schedule_exps12
  require_lemma radix2_32_schedule_exps12E
  require_lemma radix2_32_schedule_exp12E
  require_operation radix2_32_parent_exps24
  require_lemma radix2_16_parent_exps24E
  require_lemma radix2_16_parent_scheduleE32
  require_lemma radix2_32_schedule_exp_terminalE
  require_lemma invntt_radix2_32_child_scheduleE
  require_lemma radix2_32_schedule_reverse_sum_288
  require_lemma invntt_radix2_32_schedule_sumE
  require_lemma invntt_radix2_32_schedule_nonneg
  require_lemma invntt_radix2_32_child_exp_nonneg
  require_lemma invntt_radix2_32_root_cancelE
  require_lemma invntt_radix2_16_semantics_child_left
  require_lemma invntt_radix2_16_semantics_child_right
  require_lemma invntt_radix2_32_segment_leftE
  require_operation invntt_radix2_32_scaled_diff_poly
  require_lemma invntt_radix2_32_scaled_diff_polyE
  require_lemma segment_poly_scaled_diff2_32E
  require_lemma invntt_radix2_32_segment_rightE
  require_lemma invntt_radix2_32_output_segmentE
  require_lemma invntt_radix2_32_recombine_factor_eqm
  require_operation doubled_four_times
  require_operation invntt_radix2_32_semantics
  require_lemma invntt_radix2_32_algebra_refines_invntt_radix2_16_semantics
  require_lemma invntt_radix2_32_spec_semantics
  require_lemma invntt_radix2_32_semantics_functional
  require_lemma invntt_radix2_32_correct_semantics
  reject_proof_holes

  # The implementation verifier is intentionally a separate top-level command.
  # This Make dependency chain recompiles the step4, step8, step16, and step32
  # semantic theories without rerunning the separate implementation verifiers.
  compile_easycrypt_via_makefile

  printf 'PASS: inverse step4, step8, step16, and step32 semantic dependencies recompile before step32\n'
  printf 'PASS: inverse step32 pairs adjacent degree-32 child residues into degree-64 parents\n'
  printf 'PASS: each inverse radix2_32 block reconstructs one degree-64 residue of 16p\n'
  printf 'PASS: semantic endpoints typecheck against the inverse radix2_32 Jasmin proof\n'
  printf 'PASS: EasyCrypt NTRU+768 inverse radix2_32 polynomial semantics\n'
}
