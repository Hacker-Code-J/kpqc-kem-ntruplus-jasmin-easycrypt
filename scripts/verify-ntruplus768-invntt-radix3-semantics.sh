...
PROOF_FILE="$PROOF_DIR/NTRUPlus768InverseNTTRadix3Semantics.ec"
RECOMBINATION_FILE="$PROOF_DIR/NTRUPlus768InverseNTTRadix3Recombination.ec"
INTERPOLATION_FILE="$PROOF_DIR/NTRUPlus768InverseNTTRadix3Interpolation.ec"
...
RADIX3_PROOF_CACHE="${PROOF_FILE%.ec}.eco"
RADIX3_RECOMBINATION_CACHE="${RECOMBINATION_FILE%.ec}.eco"
RADIX3_INTERPOLATION_CACHE="${INTERPOLATION_FILE%.ec}.eco"
...
WORKDIR=$(mktemp -d "$TMPDIR_PARENT/ntruplus768-invntt-radix3-semantics.XXXXXX")

cleanup() {
  rm -f "$RADIX3_PROOF_CACHE" "$RADIX3_RECOMBINATION_CACHE" \
...
reject_proof_holes() {
  ...
  if grep -ERni --include='*.ec' \
    ...
    fail "inverse-NTT radix3 semantics proof tree contains a proof-hole keyword"
  else
    status=$?
...
compile_easycrypt_via_makefile() {
  ...
  if ! make -C "$PROOF_DIR" inverse-radix3 EASYCRYPT="$easycrypt_bin"; then
    fail "EasyCrypt inverse-NTT radix3 semantics proof failed"
  fi
}
...
main() {
  ...
  [[ -f "$STEP64_PROOF_FILE" ]] ||
    fail "missing predecessor EasyCrypt proof: $STEP64_PROOF_FILE"
  ...
  bash -n "$0"
  require_operation invntt_radix3_semantics
  require_lemma invntt_radix3_algebra_refines_invntt_radix2_64_semantics
  require_lemma invntt_radix3_spec_semantics
  require_lemma invntt_radix3_semantics_functional
  require_lemma invntt_radix3_correct_semantics
  reject_proof_holes
  ...
  # step64, and radix3 semantic theories without rerunning implementation checks.
  compile_easycrypt_via_makefile

  printf 'PASS: inverse step4, step8, step16, step32, step64, and radix3 semantic dependencies recompile before radix3\n'
  printf 'PASS: inverse radix3 recombines six degree-128 residues into two degree-384 residues\n'
  printf 'PASS: the two radix3 outputs represent 96p at root exponents 96 and 480\n'
  printf 'PASS: semantic endpoints typecheck against the inverse radix3 Jasmin proof\n'
  printf 'PASS: EasyCrypt NTRU+768 inverse radix3 polynomial semantics\n'
}
