...
EXPECTED_ZETAS = [1, -722, -723, -257, -1124, -867]
EXPECTED_EXPORT = "jade_ntruplus_ntruplus768_amd64_ref_ntt_radix2_64"
EXPECTED_HELPER = "__radix2_64_block"
...
def verify_c_text(text: str) -> None:
    ...
    require(
        zetas[6:12] == EXPECTED_ZETAS,
        f"expected C zetas[6:12] == {EXPECTED_ZETAS}, found {zetas[6:12]}",
    )
    ...
    require(
        ...
        "C ntt() prefix no longer proves that the step=64 layer starts at zetas[6]",
    )
    ...
    require(
        ...
        "C ntt() radix-2 tail no longer matches the verified step=64 schedule and Barrett butterfly data flow",
    )
    require(
...
def verify_jasmin_text(text: str) -> None:
    ...
    for index, value in enumerate(EXPECTED_ZETAS, start=6):
        require(
    ...
    helper_pattern = re.compile(
        ...
        r"rp\[i \+ 64\] = __barrett_reduce\(diff\); rp\[i\] = __barrett_reduce\(sum\); "
        r"i \+= 1; \} return rp;"
    )
    require(
        helper_pattern.search(helper_body) is not None,
        "Jasmin radix-2 helper no longer matches the verified pair offset, Montgomery multiply, and Barrett butterfly flow",
    )

    export_body = compact(extract_function_body(text, EXPECTED_EXPORT))
    call_pattern = re.compile(
        r"_ = #init_msf\(\); "
        r"rp = __radix2_64_block\(rp, 0, NTRUPLUS_ZETA6\); "
        r"rp = __radix2_64_block\(rp, 128, NTRUPLUS_ZETA7\); "
        r"rp = __radix2_64_block\(rp, 256, NTRUPLUS_ZETA8\); "
        r"rp = __radix2_64_block\(rp, 384, NTRUPLUS_ZETA9\); "
        r"rp = __radix2_64_block\(rp, 512, NTRUPLUS_ZETA10\); "
        r"rp = __radix2_64_block\(rp, 640, NTRUPLUS_ZETA11\); "
        r"return rp;"
    )
    require(
        call_pattern.search(export_body) is not None,
        "Jasmin radix-2 export no longer matches the verified six-block schedule or helper-call order",
    )
...
def run_self_check(c_text: str, jasmin_text: str) -> None:
    ...
    mutated_c_start = c_text.replace("int k = 1;", "int k = 2;", 1)
    require(mutated_c_start != c_text, "could not construct C initial-k mutation")
    expect_rejected(verify_c_text, mutated_c_start, "C step64 layer starts at the wrong schedule index")
    ...
    require(mutated_c_zeta_window != c_text, "could not construct C step64 zeta-order mutation")
    expect_rejected(verify_c_text, mutated_c_zeta_window, "C wrong step64 zeta order")
    ...
    mutated_jasmin_zeta = jasmin_text.replace("NTRUPLUS_ZETA8 = -723", "NTRUPLUS_ZETA8 = -257", 1)
    ...
    mutated_jasmin_offset = jasmin_text.replace("high = rp[i + 64];", "high = rp[i + 32];", 1)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Tie the standalone Jasmin NTRU+768 radix-2 step64 slice to the C tail layer."
    )
    parser.add_argument("--c-source", required=True, help="path to NTRU+/NTRU+768/ntt.c")
    parser.add_argument("--jasmin-source", required=True, help="path to ntt_radix2_64.jazz")
...
def main(argv: list[str]) -> int:
    ...
    if args.self_check:
        print("PASS: NTRU+768 NTT radix-2 step64 checker rejected representative bad mutations")
    print("PASS: NTRU+768 NTT radix-2 step64 C/Jasmin source coupling")
    return 0
