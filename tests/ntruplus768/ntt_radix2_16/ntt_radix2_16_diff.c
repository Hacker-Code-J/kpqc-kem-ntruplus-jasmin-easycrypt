...
static void require_centered_step32_shape(const char *tag,
                                          size_t case_index,
                                          const int16_t values[NTRUPLUS_N])
{
    for (size_t i = 0; i < NTRUPLUS_N; ++i) {
        if (values[i] < -NTRUPLUS_QHALF || values[i] > NTRUPLUS_QHALF) {
            fprintf(stderr,
                    "%s[%zu] violates centered step32 bound at coeff %zu: %d\n",
                    tag,
                    case_index,
                    i,
                    values[i]);
...
static int16_t sample_centered_step32_value(uint32_t *state)
{
...
static void check_radix2_16_case(const char *tag,
                                 size_t case_index,
                                 const int16_t input[NTRUPLUS_N])
{
    ...
    require_centered_step32_shape(tag, case_index, input);
    memcpy(expected, input, sizeof(expected));
    memcpy(actual, input, sizeof(actual));
    ntt_radix2_16_ref(expected);
    jade_ntruplus_ntruplus768_amd64_ref_ntt_radix2_16(actual);
...
static void check_prefix_case(size_t case_index, const int16_t input[NTRUPLUS_N])
{
    ...
    require_centered_step32_shape("prefix-shape", case_index, expected);
    ntt_radix2_16_ref(expected);
    ...
    compare_output("stage1+radix3+step64+step32+step16-disjoint", case_index, expected, disjoint);
    ...
    compare_output("stage1+radix3+step64+step32+step16-in-place", case_index, expected, inplace);
}
...
static void run_boundary_cases(void)
{
    ...
    const size_t sparse_indices[] = {
        0, 15, 16, 31, 32, 47, 48, 63, 64, 79, 80, 95, 384, 399, 400, 415, 736, 751, 752, 767};

    fill_constant_centered_range(input, 0);
    check_radix2_16_case("boundary", 0, input);

    fill_constant_centered_range(input, -NTRUPLUS_QHALF);
    check_radix2_16_case("boundary", 1, input);

    fill_constant_centered_range(input, NTRUPLUS_QHALF);
    check_radix2_16_case("boundary", 2, input);

    fill_alternating_centered_range(input);
    check_radix2_16_case("boundary", 3, input);
    ...
    for (size_t sparse_case = 0; sparse_case < sizeof(sparse_indices) / sizeof(sparse_indices[0]); ++sparse_case) {
        ...
        check_radix2_16_case("boundary", 4 + sparse_case, input);
    }
}

static void run_random_radix2_16_cases(void)
{
    ...
    for (size_t case_index = 0; case_index < random_cases; ++case_index) {
        ...
        for (size_t i = 0; i < NTRUPLUS_N; ++i) {
            input[i] = sample_centered_step32_value(&state);
        }
        check_radix2_16_case("random-step16", case_index, input);
    }
}
...
{
    run_boundary_cases();
    run_random_radix2_16_cases();
    run_random_prefix_cases();
    printf("ntt radix-2 step16 differential passed: 24 boundary + 4096 direct + "
           "2048 chained stage1/radix3/step64/step32 vectors; chained disjoint "
           "and in-place\n");
    return 0;
}
