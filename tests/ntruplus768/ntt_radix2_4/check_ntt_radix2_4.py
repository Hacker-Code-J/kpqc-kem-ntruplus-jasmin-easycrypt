...
EXPECTED_EXPORT = "jade_ntruplus_ntruplus768_amd64_ref_ntt_radix2_4"
EXPECTED_HELPER = "__radix2_4_block"
...
def parse_tail_schedule() -> tuple[int, int, list[int]]:
    ...
    start_index = 6 + step64_blocks + step32_blocks + step16_blocks + step8_blocks
    return start_index, step4_blocks, [block * (EXPECTED_OFFSET << 1) for block in range(step4_blocks)]
...
def verify_c_text(text: str) -> None:
    ...
    require(
        zetas[96:192] == EXPECTED_ZETAS,
        f"expected C zetas[96:192] == {EXPECTED_ZETAS}, found {zetas[96:192]}",
    )
    ...
    require(
        ...
        "C ntt() prefix no longer proves that the standalone step=4 layer starts after stage1/radix3/step64/step32/step16/step8",
    )
    ...
    require(start_index == 96, f"derived C step4 zeta start changed from 96 to {start_index}")
    require(block_count == 96, f"derived C step4 block count changed from 96 to {block_count}")
    require(bases == EXPECTED_BASES, f"derived C step4 bases changed: expected {EXPECTED_BASES}, found {bases}")
...
def verify_jasmin_text(text: str) -> None:
    ...
    for index, value in enumerate(EXPECTED_ZETAS, start=96):
        require(
    ...
    helper_pattern = re.compile(
        ...
        r"rp\[i \+ 4\] = __barrett_reduce\(diff\); rp\[i\] = __barrett_reduce\(sum\); "
        r"i \+= 1; \} return rp;"
    )
    require(
        helper_pattern.search(helper_body) is not None,
        "Jasmin radix-2 helper no longer matches the verified step=4 pair offset, multiply, and Barrett flow",
    )

    export_body = compact(extract_function_body(text, EXPECTED_EXPORT))
    expected_calls = " ".join(
        f"rp = {EXPECTED_HELPER}(rp, {base}, NTRUPLUS_ZETA{index});"
        for index, base in enumerate(EXPECTED_BASES, start=96)
    )
    call_pattern = re.compile(rf"_ = #init_msf\(\); {re.escape(expected_calls)} return rp;")
    require(
        call_pattern.search(export_body) is not None,
        "Jasmin radix-2 export no longer matches the verified 96-block step=4 schedule",
    )
...
def run_self_check(c_text: str, jasmin_text: str) -> None:
    ...
    require(mutated_c_zeta_window != c_text, "could not construct C step4 zeta-order mutation")
    expect_rejected(verify_c_text, mutated_c_zeta_window, "C wrong step4 zeta order")
    ...
    require(mutated_c_tail != c_text, "could not construct C tail-depth mutation")
    expect_rejected(verify_c_text, mutated_c_tail, "C radix-2 tail no longer reaches the verified step4 layer")
    ...
    mutated_jasmin_zeta = jasmin_text.replace("NTRUPLUS_ZETA144 = 361", "NTRUPLUS_ZETA144 = 230", 1)
    ...
    mutated_jasmin_offset = jasmin_text.replace("high = rp[i + 4];", "high = rp[i + 8];", 1)
    ...
    mutated_jasmin_barrett = jasmin_text.replace(
        "rp[i + 4] = __barrett_reduce(diff);",
        "rp[i + 4] = __barrett_reduce(sum);",
        1,
    )
    ...
    mutated_jasmin_schedule = jasmin_text.replace(
        "rp = __radix2_4_block(rp, 376, NTRUPLUS_ZETA143);\n"
        "  rp = __radix2_4_block(rp, 384, NTRUPLUS_ZETA144);",
        "rp = __radix2_4_block(rp, 384, NTRUPLUS_ZETA144);\n"
        "  rp = __radix2_4_block(rp, 376, NTRUPLUS_ZETA143);",
        1,
    )
    ...
    stale_step8_copy = jasmin_text.replace("stop += 4;", "stop += 8;", 1)
    stale_step8_copy = stale_step8_copy.replace("high = rp[i + 4];", "high = rp[i + 8];", 1)
    stale_step8_copy = stale_step8_copy.replace("rp[i + 4] = __barrett_reduce(diff);", "rp[i + 8] = __barrett_reduce(diff);", 1)
    stale_step8_copy = stale_step8_copy.replace(
        "rp = __radix2_4_block(rp, 384, NTRUPLUS_ZETA144);",
        "rp = __radix2_4_block(rp, 768, NTRUPLUS_ZETA144);",
        1,
    )
    require(stale_step8_copy != jasmin_text, "could not construct stale step8-shaped Jasmin mutation")
    expect_rejected(verify_jasmin_text, stale_step8_copy, "Jasmin accepted stale step8-shaped copy")


def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Tie the standalone Jasmin NTRU+768 radix-2 step4 slice to the C tail layer."
    )
    parser.add_argument("--c-source", required=True, help="path to NTRU+/NTRU+768/ntt.c")
    parser.add_argument("--jasmin-source", required=True, help="path to ntt_radix2_4.jazz")
...
def main(argv: list[str]) -> int:
    ...
    if args.self_check:
        print("PASS: NTRU+768 NTT radix-2 step4 checker rejected representative bad mutations")
    print("PASS: NTRU+768 NTT radix-2 step4 C/Jasmin source coupling")
    return 0
