...
def test_vectors() -> list[tuple[str, bytes]]:
    vectors = [
        ...
        ("middle-bit", bytes(INPUT_BYTES // 2) + b"\x01" + bytes(INPUT_BYTES // 2 - 1)),
    ]
...
def verify(binary: Path) -> None:
    if not binary.is_file():
        raise VerificationError(f"missing hash_h driver: {binary}")
    ...
    for label, message in test_vectors():
        ...
        if repeated.returncode != 0:
            ...
            raise VerificationError(
                f"driver repeat failed for {label} with exit {repeated.returncode}: {detail}",
            )
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare NTRU+768 hash_h against Python's independent SHAKE256 oracle",
    )
...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        print(f"hash_h differential test failed: {exc}", file=sys.stderr)
        return 1

    print(f"hash_h differential test passed ({len(test_vectors())} vectors)")
    return 0
