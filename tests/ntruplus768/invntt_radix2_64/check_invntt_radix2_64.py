...
EXPECTED_EXPORT = "jade_ntruplus_ntruplus768_amd64_ref_invntt_radix2_64"
EXPECTED_HELPER = "__invntt_radix2_64_block"
EXPECTED_BASES = list(range(0, 768, 128))
EXPECTED_OFFSET = 64
EXPECTED_C_START = 6
EXPECTED_PARAM_START = 6
EXPECTED_PARAM_STOP = 11
MISSING_SOURCE_HINT = (
    "expected Jasmin source implementing export "
    "`jade_ntruplus_ntruplus768_amd64_ref_invntt_radix2_64` at "
    "ntruplus/jasmin/768/ref/invntt_radix2_64.jazz with signature "
...
def parse_inv_tail_schedule() -> tuple[int, int, list[int]]:
    ...
    step64_blocks = 768 // (2 * EXPECTED_OFFSET)
    start_index = 191 - step4_blocks - step8_blocks - step16_blocks - step32_blocks - step64_blocks + 1
    return start_index, step64_blocks, [block * (EXPECTED_OFFSET << 1) for block in range(step64_blocks)]
...
def verify_c_text(text: str) -> None:
    ...
    require(
        list(reversed(zetas[6:12])) == EXPECTED_ZETAS,
        f"expected reversed C zetas[6:12] == {EXPECTED_ZETAS}, found {list(reversed(zetas[6:12]))}",
    )
    ...
    require(start_index == EXPECTED_C_START, f"derived C inverse step64 zeta start changed from {EXPECTED_C_START} to {start_index}")
    require(block_count == 6, f"derived C inverse step64 block count changed from 6 to {block_count}")
    require(bases == EXPECTED_BASES, f"derived C inverse step64 bases changed: expected {EXPECTED_BASES}, found {bases}")
...
def verify_jasmin_text(text: str) -> None:
    ...
    helper_pattern = re.compile(
        ...
        r"i = base; stop = base; stop \+= 64; while \(i < stop\) \{ "
        r"low = rp\[i\]; high = rp\[i \+ 64\]; "
        ...
        r"rp\[i\] = sum; rp\[i \+ 64\] = prod; i \+= 1; \} return rp;"
    )
    require(
        helper_pattern.search(helper_body) is not None,
        "Jasmin inverse radix-2 helper no longer matches the verified in-place step=64 flow",
    )
    ...
    require(
        ...
        "Jasmin inverse radix-2 export no longer matches the verified 6-block descending-zeta schedule",
    )
...
def run_self_check(c_text: str) -> None:
    ...
    mutated_c_zeta_window = c_text.replace("    1,  -722,\n\t -723,  -257, -1124,  -867,", " -722,     1,\n\t -723,  -257, -1124,  -867,", 1)
    require(mutated_c_zeta_window != c_text, "could not construct C inverse step64 zeta-order mutation")
    expect_rejected(verify_c_text, mutated_c_zeta_window, "C wrong inverse step64 zeta order")
    ...
    expect_rejected(verify_c_text, mutated_c_tail, "C inverse radix-2 head no longer reaches the verified step64 layer")
    ...
    mutated_jasmin_zeta = EXPECTED_JASMIN_SOURCE.replace("NTRUPLUS_ZETA6 = 1", "NTRUPLUS_ZETA6 = -722", 1)
    ...
    mutated_jasmin_offset = EXPECTED_JASMIN_SOURCE.replace("high = rp[i + 64];", "high = rp[i + 32];", 1)
    ...
    mutated_jasmin_schedule = EXPECTED_JASMIN_SOURCE.replace(
        f"  rp = {EXPECTED_HELPER}(rp, 0, NTRUPLUS_ZETA11);\n"
        f"  rp = {EXPECTED_HELPER}(rp, 128, NTRUPLUS_ZETA10);",
        f"  rp = {EXPECTED_HELPER}(rp, 128, NTRUPLUS_ZETA10);\n"
        f"  rp = {EXPECTED_HELPER}(rp, 0, NTRUPLUS_ZETA11);",
        1,
    )
    ...
    stale_two_buffer_copy = stale_two_buffer_copy.replace("high = rp[i + 64];", "high = ap[i + 64];", 1)
    ...
    stale_two_buffer_copy = stale_two_buffer_copy.replace(
        f"  rp = {EXPECTED_HELPER}(rp, 0, NTRUPLUS_ZETA11);",
        f"  rp = {EXPECTED_HELPER}(rp, ap, 0, NTRUPLUS_ZETA11);",
        1,
    )
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Tie the standalone Jasmin NTRU+768 inverse radix-2 step64 slice to the C inverse loop."
    )
    parser.add_argument("--c-source", required=True, help="path to NTRU+/NTRU+768/ntt.c")
    parser.add_argument("--jasmin-source", required=True, help="path to invntt_radix2_64.jazz")
...
def main(argv: list[str]) -> int:
    ...
    if args.self_check:
        print("PASS: NTRU+768 inverse radix-2 step64 checker rejected representative bad mutations")
    print("PASS: NTRU+768 inverse radix-2 step64 C/Jasmin source coupling")
    return 0
