...
EXPECTED_EXPORT = "jade_ntruplus_ntruplus768_amd64_ref_invntt_radix2_32"
EXPECTED_HELPER = "__invntt_radix2_32_block"
EXPECTED_BASES = list(range(0, 768, 64))
EXPECTED_OFFSET = 32
EXPECTED_C_START = 12
EXPECTED_PARAM_START = 12
EXPECTED_PARAM_STOP = 23
MISSING_SOURCE_HINT = (
    "expected Jasmin source implementing export "
    "`jade_ntruplus_ntruplus768_amd64_ref_invntt_radix2_32` at "
    "ntruplus/jasmin/768/ref/invntt_radix2_32.jazz with signature "
...
def parse_inv_tail_schedule() -> tuple[int, int, list[int]]:
    ...
    step32_blocks = 768 // (2 * EXPECTED_OFFSET)
    start_index = 191 - step4_blocks - step8_blocks - step16_blocks - step32_blocks + 1
    return start_index, step32_blocks, [block * (EXPECTED_OFFSET << 1) for block in range(step32_blocks)]
...
def verify_c_text(text: str) -> None:
    ...
    require(
        list(reversed(zetas[12:24])) == EXPECTED_ZETAS,
        f"expected reversed C zetas[12:24] == {EXPECTED_ZETAS}, found {list(reversed(zetas[12:24]))}",
    )
    ...
    require(start_index == EXPECTED_C_START, f"derived C inverse step32 zeta start changed from {EXPECTED_C_START} to {start_index}")
    require(block_count == 12, f"derived C inverse step32 block count changed from 12 to {block_count}")
    require(bases == EXPECTED_BASES, f"derived C inverse step32 bases changed: expected {EXPECTED_BASES}, found {bases}")
...
def verify_jasmin_text(text: str) -> None:
    ...
    helper_pattern = re.compile(
        ...
        r"rp\[i\] = sum; rp\[i \+ 32\] = prod; i \+= 1; \} return rp;"
    )
    require(
        helper_pattern.search(helper_body) is not None,
        "Jasmin inverse radix-2 helper no longer matches the verified in-place step=32 flow",
    )
    ...
    require(
        ...
        "Jasmin inverse radix-2 export no longer matches the verified 12-block descending-zeta schedule",
    )
...
def run_self_check(c_text: str) -> None:
    ...
    require(mutated_c_zeta_window != c_text, "could not construct C inverse step32 zeta-order mutation")
    expect_rejected(verify_c_text, mutated_c_zeta_window, "C wrong inverse step32 zeta order")
    ...
    expect_rejected(verify_c_text, mutated_c_tail, "C inverse radix-2 head no longer reaches the verified step32 layer")
    ...
    mutated_jasmin_zeta = EXPECTED_JASMIN_SOURCE.replace("NTRUPLUS_ZETA12 = -256", "NTRUPLUS_ZETA12 = 1484", 1)
    ...
    mutated_jasmin_offset = EXPECTED_JASMIN_SOURCE.replace("high = rp[i + 32];", "high = rp[i + 64];", 1)
    ...
    expect_rejected(verify_jasmin_text, mutated_jasmin_polarity, "Jasmin inverse helper flips high-minus-low polarity")

    mutated_jasmin_schedule = EXPECTED_JASMIN_SOURCE.replace(
        f"  rp = {EXPECTED_HELPER}(rp, 0, NTRUPLUS_ZETA23);\n"
        f"  rp = {EXPECTED_HELPER}(rp, 64, NTRUPLUS_ZETA22);",
        f"  rp = {EXPECTED_HELPER}(rp, 64, NTRUPLUS_ZETA22);\n"
        f"  rp = {EXPECTED_HELPER}(rp, 0, NTRUPLUS_ZETA23);",
        1,
    )
    ...
    stale_two_buffer_copy = stale_two_buffer_copy.replace("high = rp[i + 32];", "high = ap[i + 32];", 1)
    ...
    stale_two_buffer_copy = stale_two_buffer_copy.replace(
        f"  rp = {EXPECTED_HELPER}(rp, 0, NTRUPLUS_ZETA23);",
        f"  rp = {EXPECTED_HELPER}(rp, ap, 0, NTRUPLUS_ZETA23);",
        1,
    )
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Tie the standalone Jasmin NTRU+768 inverse radix-2 step32 slice to the C inverse loop."
    )
    parser.add_argument("--c-source", required=True, help="path to NTRU+/NTRU+768/ntt.c")
    parser.add_argument("--jasmin-source", required=True, help="path to invntt_radix2_32.jazz")
...
def main(argv: list[str]) -> int:
    ...
    if args.self_check:
        print("PASS: NTRU+768 inverse radix-2 step32 checker rejected representative bad mutations")
    print("PASS: NTRU+768 inverse radix-2 step32 C/Jasmin source coupling")
    return 0
