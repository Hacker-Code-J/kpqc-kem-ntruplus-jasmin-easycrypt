...
static void fill_step32_pair_stress(int16_t dst[NTRUPLUS_N])
{
    for (size_t block = 0; block < 12; ++block) {
        const size_t base = block * 64;
        for (size_t lane = 0; lane < 32; ++lane) {
            dst[base + lane] = (lane & 1u) == 0 ? (NTRUPLUS_Q - 1) : -NTRUPLUS_Q;
            dst[base + 32 + lane] = (lane & 1u) == 0 ? -NTRUPLUS_Q : (NTRUPLUS_Q - 1);
        }
    }
}

static void fill_step32_block_gradient(int16_t dst[NTRUPLUS_N])
{
    for (size_t block = 0; block < 12; ++block) {
        const size_t base = block * 64;
        for (size_t lane = 0; lane < 32; ++lane) {
            dst[base + lane] = (int16_t)((int)(3 * block) - (int)lane);
            dst[base + 32 + lane] = (int16_t)((int)lane - (int)(2 * block));
        }
    }
}
                                ...
                                const int16_t input[NTRUPLUS_N])
{
    ...
    invntt_radix2_32_ref(expected, input);
                            ...
                            const int16_t input[NTRUPLUS_N])
{
    ...
    invntt_radix2_32_ref(expected, step16_out);
...
int main(void)
{
    ...
    fill_step32_pair_stress(standalone_vectors[4]);
    fill_step32_block_gradient(standalone_vectors[5]);
    ...
    fill_step32_pair_stress(prefix_vectors[4]);
    fill_step32_block_gradient(prefix_vectors[5]);

    for (size_t i = 0; i < 6; ++i) {
        run_standalone_case("invntt-radix2-32", i, standalone_vectors[i]);
        run_prefix_case("invntt-radix2-4-then-8-then-16-then-32", i, prefix_vectors[i]);
    }

    for (size_t i = 0; i < STANDALONE_RANDOM_CASES; ++i) {
        const uint32_t seed = 0x243f6a88u ^ ((uint32_t)i * 0x9e3779b9u);

        fill_random_qrange(random_input, seed);
        run_standalone_case("invntt-radix2-32-random", i, random_input);
    }

    for (size_t i = 0; i < PREFIX_RANDOM_CASES; ++i) {
        const uint32_t seed = 0xb7e15162u ^ ((uint32_t)i * 0x85ebca6bu);

        fill_random_qrange(random_input, seed);
        run_prefix_case("invntt-radix2-4-then-8-then-16-then-32-random", i, random_input);
    }

    printf("PASS: NTRU+768 inverse radix-2 step32 differential checks: "
