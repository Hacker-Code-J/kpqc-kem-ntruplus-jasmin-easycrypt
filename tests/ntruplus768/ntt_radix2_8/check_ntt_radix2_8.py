...
EXPECTED_EXPORT = "jade_ntruplus_ntruplus768_amd64_ref_ntt_radix2_8"
EXPECTED_HELPER = "__radix2_8_block"
...
def parse_tail_schedule() -> tuple[int, int, list[int]]:
    ...
    start_index = 6 + step64_blocks + step32_blocks + step16_blocks
...
def verify_c_text(text: str) -> None:
    ...
    require(
        zetas[48:96] == EXPECTED_ZETAS,
        f"expected C zetas[48:96] == {EXPECTED_ZETAS}, found {zetas[48:96]}",
    )
    ...
    require(
        ...
        "C ntt() prefix no longer proves that the standalone step=8 layer starts after stage1/radix3/step64/step32/step16",
    )
    ...
    require(start_index == 48, f"derived C step8 zeta start changed from 48 to {start_index}")
    require(block_count == 48, f"derived C step8 block count changed from 48 to {block_count}")
    require(bases == EXPECTED_BASES, f"derived C step8 bases changed: expected {EXPECTED_BASES}, found {bases}")
...
def verify_jasmin_text(text: str) -> None:
    ...
    for index, value in enumerate(EXPECTED_ZETAS, start=48):
        require(
    ...
    helper_pattern = re.compile(
        ...
        r"rp\[i \+ 8\] = __barrett_reduce\(diff\); rp\[i\] = __barrett_reduce\(sum\); "
        r"i \+= 1; \} return rp;"
    )
    require(
        helper_pattern.search(helper_body) is not None,
        "Jasmin radix-2 helper no longer matches the verified step=8 pair offset, multiply, and Barrett flow",
    )

    export_body = compact(extract_function_body(text, EXPECTED_EXPORT))
    expected_calls = " ".join(
        f"rp = {EXPECTED_HELPER}(rp, {base}, NTRUPLUS_ZETA{index});"
        for index, base in enumerate(EXPECTED_BASES, start=48)
    )
    call_pattern = re.compile(rf"_ = #init_msf\(\); {re.escape(expected_calls)} return rp;")
    require(
        call_pattern.search(export_body) is not None,
        "Jasmin radix-2 export no longer matches the verified 48-block step=8 schedule",
    )
...
def run_self_check(c_text: str, jasmin_text: str) -> None:
    ...
    require(mutated_c_zeta_window != c_text, "could not construct C step8 zeta-order mutation")
    expect_rejected(verify_c_text, mutated_c_zeta_window, "C wrong step8 zeta order")
    ...
    expect_rejected(verify_c_text, mutated_c_tail, "C radix-2 tail no longer reaches the verified step8 layer")
    ...
    mutated_jasmin_zeta = jasmin_text.replace("NTRUPLUS_ZETA80 = 675", "NTRUPLUS_ZETA80 = -834", 1)
    ...
    mutated_jasmin_offset = jasmin_text.replace("high = rp[i + 8];", "high = rp[i + 16];", 1)
    ...
    mutated_jasmin_barrett = jasmin_text.replace(
        "rp[i + 8] = __barrett_reduce(diff);",
        "rp[i + 8] = __barrett_reduce(sum);",
        1,
    )
    ...
    mutated_jasmin_schedule = jasmin_text.replace(
        "rp = __radix2_8_block(rp, 240, NTRUPLUS_ZETA63);\n"
        "  rp = __radix2_8_block(rp, 256, NTRUPLUS_ZETA64);",
        "rp = __radix2_8_block(rp, 256, NTRUPLUS_ZETA64);\n"
        "  rp = __radix2_8_block(rp, 240, NTRUPLUS_ZETA63);",
        1,
    )
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Tie the standalone Jasmin NTRU+768 radix-2 step8 slice to the C tail layer."
    )
    parser.add_argument("--c-source", required=True, help="path to NTRU+/NTRU+768/ntt.c")
    parser.add_argument("--jasmin-source", required=True, help="path to ntt_radix2_8.jazz")
...
def main(argv: list[str]) -> int:
    ...
    if args.self_check:
        print("PASS: NTRU+768 NTT radix-2 step8 checker rejected representative bad mutations")
    print("PASS: NTRU+768 NTT radix-2 step8 C/Jasmin source coupling")
    return 0
