...
def verify_poly_source_text(poly_text: str) -> None:
    ...
    require(
        ...
        "poly_ntt() must keep the exact forwarding call into ntt()",
    )

    tobytes_signature = extract_function_signature(poly_text, "poly_tobytes")
    require(
        tobytes_signature == "void poly_tobytes(uint8_t r[NTRUPLUS_POLYBYTES], const poly *a)",
        f"unexpected poly_tobytes signature: {tobytes_signature}",
    )
    tobytes_body = compact(extract_function_body(poly_text, "poly_tobytes"))
    expected_tobytes = (
    ...
    require(
        tobytes_body == expected_tobytes,
        "poly_tobytes() must keep the exact 384-triplet serialization loop",
    )
...
def verify_jasmin_text(text: str) -> None:
    ...
    require(
        "t0 = ap[2 * i];" in export_body
        and "t1 = ap[2 * i + 1];" in export_body
        and "rp[off] = t0;" in export_body
        and "rp[off + 1] = t;" in export_body
        and "rp[off + 2] = t;" in export_body,
        "Jasmin poly_tobytes() must keep the paired-coefficient triplet stores",
    )
...
def verify_kem_text(kem_text: str) -> None:
    ...
    require(body.count(verify_call) == 1, "crypto_kem_dec() must compare buf1 and buf2 exactly once after serialization")
    require(
        f"{hash_call} {cbd_call} {ntt_call} {tobytes_call} {verify_call}" in body,
        "crypto_kem_dec() must keep exact hash_h -> poly_cbd1 -> poly_ntt -> poly_tobytes -> verify order for r1",
    )
...
def expect_rejected(
    ...
    try:
        verify_all(params_text, poly_header_text, poly_source_text, kem_text, jasmin_text)
...
def run_self_check(
    ...
    source_mutations = (
        ...
        ("t[0] += (t[0] >> 15) & NTRUPLUS_Q;", "t[0] += (t[0] >> 15) & (NTRUPLUS_Q - 1);", "source tobytes t0 sign-fix"),
        ("r[3*i+1] = (t[0] >> 8) | (t[1] << 4);", "r[3*i+1] = (t[0] >> 8) | (t[1] << 3);", "source tobytes middle-byte packing"),
        ("r[3*i+2] = (t[1] >> 4);", "r[3*i+2] = (t[1] >> 5);", "source tobytes high-byte packing"),
    )
    ...
    jasmin_mutations = (
        ...
        (
            "rp[off + 1] = t;",
            "rp[off + 1] = t0;",
            "jasmin middle-byte store",
        ),
        (
            "rp[off + 2] = t;",
            "rp[off + 2] = t0;",
            "jasmin high-byte store",
        ),
    )
    for old, new, label in jasmin_mutations:
        expect_rejected(
            params_text,
            poly_header_text,
            poly_source_text,
            kem_text,
            replace_once(jasmin_text, old, new, label),
            label,
        )
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Fail-closed checker for the NTRU+768 decap r1 post-NTT -> poly_tobytes(buf2) seam",
    )
...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        verify_all(params_text, poly_header_text, poly_source_text, kem_text, jasmin_text)
        if args.self_check:
            run_self_check(params_text, poly_header_text, poly_source_text, kem_text, jasmin_text)
    except (OSError, VerificationError) as exc:
        print(f"decap-r1 tobytes checker failed: {exc}", file=sys.stderr)
        return 1

    print("decap-r1 tobytes checker passed")
    if args.self_check:
        print("decap-r1 tobytes checker self-check passed")
    return 0
