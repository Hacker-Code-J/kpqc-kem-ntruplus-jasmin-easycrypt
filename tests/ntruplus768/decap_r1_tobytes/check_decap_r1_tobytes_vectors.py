...
def decode_driver_output(data: bytes) -> dict[str, str]:
    ...
    for key in ("target_hex", "oracle_hex", "jasmin_hex"):
...
def verify(binary: Path) -> None:
    if not binary.is_file():
        raise VerificationError(f"missing decap-r1 tobytes driver: {binary}")
    ...
    for label, tail in test_vectors():
        ...
        if payload["target_hex"] != payload["oracle_hex"]:
            raise VerificationError(f"C/oracle serialization mismatch for {label}")
        if payload["target_hex"] != payload["jasmin_hex"]:
            raise VerificationError(f"C/Jasmin serialization mismatch for {label}")
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare decap-r1 post-NTT poly_tobytes(buf2) against independent packing and Jasmin poly_tobytes",
    )
...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        print(f"decap-r1 tobytes differential test failed: {exc}", file=sys.stderr)
        return 1

    print(f"decap-r1 tobytes differential test passed ({len(test_vectors())} vectors)")
    return 0
