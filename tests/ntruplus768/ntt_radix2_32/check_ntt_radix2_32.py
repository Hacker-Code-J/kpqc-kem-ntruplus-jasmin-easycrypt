...
EXPECTED_ZETAS = [-256, 1484, 1262, -1590, 1611, 222, 1164, -1346, 1716, -1521, -357, 395]
EXPECTED_EXPORT = "jade_ntruplus_ntruplus768_amd64_ref_ntt_radix2_32"
EXPECTED_HELPER = "__radix2_32_block"
...
def verify_c_text(text: str) -> None:
    ...
    require(
        zetas[12:24] == EXPECTED_ZETAS,
        f"expected C zetas[12:24] == {EXPECTED_ZETAS}, found {zetas[12:24]}",
    )
    ...
    require(
        ...
        "C ntt() prefix no longer proves that the standalone step=32 layer starts after stage1/radix3",
    )
    ...
    require(
        ...
        "C ntt() radix-2 tail no longer matches the verified step=64→32 schedule and Barrett butterfly flow",
    )
    require(
...
def verify_jasmin_text(text: str) -> None:
    ...
    for index, value in enumerate(EXPECTED_ZETAS, start=12):
        require(
    ...
    helper_pattern = re.compile(
        ...
        r"i = base; stop = base; stop \+= 32; while \(i < stop\) \{ "
        r"low = rp\[i\]; high = rp\[i \+ 32\]; "
        r"acc = __mul_i16\(zeta, high\); t = __montgomery_reduce\(acc\); "
        r"wd = \(32s\) low; td = \(32s\) t; wd -= td; diff = wd; "
        r"wd = \(32s\) low; td = \(32s\) t; wd \+= td; sum = wd; "
        r"rp\[i \+ 32\] = __barrett_reduce\(diff\); rp\[i\] = __barrett_reduce\(sum\); "
        r"i \+= 1; \} return rp;"
    )
    require(
        helper_pattern.search(helper_body) is not None,
        "Jasmin radix-2 helper no longer matches the verified step=32 pair offset, multiply, and Barrett flow",
    )

    export_body = compact(extract_function_body(text, EXPECTED_EXPORT))
    call_pattern = re.compile(
        r"_ = #init_msf\(\); "
        r"rp = __radix2_32_block\(rp, 0, NTRUPLUS_ZETA12\); "
        r"rp = __radix2_32_block\(rp, 64, NTRUPLUS_ZETA13\); "
        r"rp = __radix2_32_block\(rp, 128, NTRUPLUS_ZETA14\); "
        r"rp = __radix2_32_block\(rp, 192, NTRUPLUS_ZETA15\); "
        r"rp = __radix2_32_block\(rp, 256, NTRUPLUS_ZETA16\); "
        r"rp = __radix2_32_block\(rp, 320, NTRUPLUS_ZETA17\); "
        r"rp = __radix2_32_block\(rp, 384, NTRUPLUS_ZETA18\); "
        r"rp = __radix2_32_block\(rp, 448, NTRUPLUS_ZETA19\); "
        r"rp = __radix2_32_block\(rp, 512, NTRUPLUS_ZETA20\); "
        r"rp = __radix2_32_block\(rp, 576, NTRUPLUS_ZETA21\); "
        r"rp = __radix2_32_block\(rp, 640, NTRUPLUS_ZETA22\); "
        r"rp = __radix2_32_block\(rp, 704, NTRUPLUS_ZETA23\); "
        r"return rp;"
    )
    require(
        call_pattern.search(export_body) is not None,
        "Jasmin radix-2 export no longer matches the verified 12-block step=32 schedule",
    )
...
def run_self_check(c_text: str, jasmin_text: str) -> None:
    ...
    mutated_c_zeta_window = c_text.replace("-256,  1484,  1262, -1590,", "1484,  -256,  1262, -1590,", 1)
    require(mutated_c_zeta_window != c_text, "could not construct C step32 zeta-order mutation")
    expect_rejected(verify_c_text, mutated_c_zeta_window, "C wrong step32 zeta order")
    ...
    expect_rejected(verify_c_text, mutated_c_tail, "C radix-2 tail skips the verified step64→32 prefix")
    ...
    mutated_jasmin_zeta = jasmin_text.replace("NTRUPLUS_ZETA18 = 1164", "NTRUPLUS_ZETA18 = -1346", 1)
    ...
    mutated_jasmin_offset = jasmin_text.replace("high = rp[i + 32];", "high = rp[i + 64];", 1)
    ...
    mutated_jasmin_barrett_bias = jasmin_text.replace(
        "td += NTRUPLUS_BARRETT_BIAS;",
        "td += NTRUPLUS_Q;",
        1,
    )
    require(
        mutated_jasmin_barrett_bias != jasmin_text,
        "could not construct Jasmin Barrett-rounding mutation",
    )
    expect_rejected(
        verify_jasmin_text,
        mutated_jasmin_barrett_bias,
        "Jasmin wrong Barrett rounding bias",
    )
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Tie the standalone Jasmin NTRU+768 radix-2 step32 slice to the C tail layer."
    )
    parser.add_argument("--c-source", required=True, help="path to NTRU+/NTRU+768/ntt.c")
    parser.add_argument("--jasmin-source", required=True, help="path to ntt_radix2_32.jazz")
...
def main(argv: list[str]) -> int:
    ...
    if args.self_check:
        print("PASS: NTRU+768 NTT radix-2 step32 checker rejected representative bad mutations")
    print("PASS: NTRU+768 NTT radix-2 step32 C/Jasmin source coupling")
    return 0
