...
require import Array768 WArray1536 NTRUPlus768NTTRadix2_32.
...
op zeta1  : W16.t = W16.of_int (-256).
op zeta2  : W16.t = W16.of_int 1484.
op zeta3  : W16.t = W16.of_int 1262.
op zeta4  : W16.t = W16.of_int (-1590).
op zeta5  : W16.t = W16.of_int 1611.
op zeta6  : W16.t = W16.of_int 222.
op zeta7  : W16.t = W16.of_int 1164.
op zeta8  : W16.t = W16.of_int (-1346).
op zeta9  : W16.t = W16.of_int 1716.
op zeta10 : W16.t = W16.of_int (-1521).
op zeta11 : W16.t = W16.of_int (-357).
op zeta12 : W16.t = W16.of_int 395.
...
lemma radix2_32_modz_sub_dvd (u m d : int) :
  m %% d = 0 => (u - m) %% d = u %% d.
proof.
  move=> Hmd.
  have Hneg : (-m) %% d = 0.
...
lemma radix2_32_truncateu16_of_int (x : int) :
...
lemma radix2_32_truncate_sigext_add (x y : W16.t) :
  truncateu16 (sigextu32 x + sigextu32 y) = x + y.
proof.
  rewrite /sigextu32 /=.
  rewrite radix2_32_truncateu16_of_int W16.of_intD.
  by rewrite !radix2_32_w16_of_sintK.
qed.

lemma radix2_32_truncate_sigext_sub (x y : W16.t) :
  truncateu16 (sigextu32 x - sigextu32 y) = x - y.
proof.
  rewrite /sigextu32 /=.
  rewrite radix2_32_truncateu16_of_int W16.of_intS.
  by rewrite !radix2_32_w16_of_sintK.
qed.

op radix2_32_mul (z a : W16.t) : W16.t =
  montgomery_reduce (mul_i16 z a).

op radix2_32_pair (z lo hi : W16.t) : W16.t * W16.t =
  let t = radix2_32_mul z hi in
  (barrett_reduce (lo + t), barrett_reduce (lo - t)).

op radix2_32_pair_at
    (p : W16.t Array768.t) (base : int) (z : W16.t) (k : int)
    : W16.t * W16.t =
  radix2_32_pair z p.[base + k] p.[base + k + block].

op radix2_32_prefix_spec
    (p : W16.t Array768.t) (base upto : int) (z : W16.t)
    : W16.t Array768.t =
  Array768.init (fun j =>
    if base <= j < base + upto then
      (radix2_32_pair_at p base z (j - base)).`1
    else if base + block <= j < base + block + upto then
      (radix2_32_pair_at p base z (j - base - block)).`2
    else p.[j]).

op radix2_32_block_spec
    (p : W16.t Array768.t) (base : int) (z : W16.t)
    : W16.t Array768.t =
  radix2_32_prefix_spec p base block z.

op radix2_32_spec (p : W16.t Array768.t) : W16.t Array768.t =
  radix2_32_block_spec
    (radix2_32_block_spec
      (radix2_32_block_spec
        (radix2_32_block_spec
          (radix2_32_block_spec
              ...
              (radix2_32_block_spec
                (radix2_32_block_spec
                  (radix2_32_block_spec
                    (radix2_32_block_spec
                      (radix2_32_block_spec
                        (radix2_32_block_spec p 0 zeta1)
                        64 zeta2)
                      128 zeta3)
                    192 zeta4)
                  256 zeta5)
...
lemma radix2_32_mul_i16_functional (a0 b0 : W16.t) :
  hoare [NTRUPlus768NTTRadix2_32.M.__mul_i16 :
    a = a0 /\ b = b0 ==> res = mul_i16 a0 b0].
proof.
  proc; auto => />.
qed.

lemma radix2_32_montgomery_reduce_functional (a0 : W32.t) :
  hoare [NTRUPlus768NTTRadix2_32.M.__montgomery_reduce :
    a = a0 ==> res = montgomery_reduce a0].
proof.
  proc; auto => />.
qed.

lemma radix2_32_barrett_reduce_functional (a0 : W16.t) :
  hoare [NTRUPlus768NTTRadix2_32.M.__barrett_reduce :
    a = a0 ==> res = barrett_reduce a0].
proof.
  proc; auto => />.
qed.

lemma radix2_32_prefix_zero
    (p : W16.t Array768.t) (base : int) (z : W16.t) :
  0 <= base <= 768 =>
  radix2_32_prefix_spec p base 0 z = p.
proof.
  move=> Hbase.
  apply Array768.tP => j Hj.
  rewrite /radix2_32_prefix_spec Array768.initiE 1:Hj.
  smt().
qed.

lemma radix2_32_prefix_curr0
    (p : W16.t Array768.t) (base upto : int) (z : W16.t) :
  0 <= base =>
  0 <= upto < block =>
  base + block + upto < 768 =>
  (radix2_32_prefix_spec p base upto z).[base + upto] = p.[base + upto].
proof.
  move=> Hbase Hupto Hbound.
  rewrite /radix2_32_prefix_spec Array768.initiE 1:/#.
  smt().
qed.

lemma radix2_32_prefix_curr1
    (p : W16.t Array768.t) (base upto : int) (z : W16.t) :
  0 <= base =>
  0 <= upto < block =>
  base + block + upto < 768 =>
  (radix2_32_prefix_spec p base upto z).[base + block + upto] =
    p.[base + block + upto].
proof.
  move=> Hbase Hupto Hbound.
  rewrite /radix2_32_prefix_spec Array768.initiE 1:/#.
...
lemma radix2_32_prefix_step
    (p : W16.t Array768.t) (base upto : int) (z : W16.t) :
  0 <= base =>
  0 <= upto < block =>
  base + block + upto < 768 =>
  (radix2_32_prefix_spec p base upto z)
    .[base + upto <- (radix2_32_pair_at p base z upto).`1]
    .[base + block + upto <- (radix2_32_pair_at p base z upto).`2] =
  radix2_32_prefix_spec p base (upto + 1) z.
proof.
  move=> Hbase Hupto Hbound.
  apply Array768.tP => j Hj.
  rewrite /radix2_32_prefix_spec.
...
lemma radix2_32_block_functional
    (rp0 : W16.t Array768.t) (base0 : int) (z0 : W16.t) :
  valid_base base0 =>
  hoare [NTRUPlus768NTTRadix2_32.M.__radix2_32_block :
    rp = rp0 /\ base = W64.of_int base0 /\ zeta_0 = z0 ==>
    res = radix2_32_block_spec rp0 base0 z0].
proof.
  move=> Hvalid.
  have [Hbase Hbound] := valid_base_bounds base0 Hvalid.
  proc.
  seq 3 :
    (rp = radix2_32_prefix_spec rp0 base0 0 z0 /\
     i = W64.of_int base0 /\
     stop = W64.of_int base0 + W64.of_int block /\
     zeta_0 = z0).
  + auto => />.
    rewrite radix2_32_prefix_zero 1:/# /block.
    smt().
  while (base0 <= W64.to_uint i <= base0 + block /\
         stop = W64.of_int base0 + W64.of_int block /\
         zeta_0 = z0 /\
         rp = radix2_32_prefix_spec rp0 base0 (W64.to_uint i - base0) z0); last first.
  + skip => &hr [Hrp [Hi [Hstop Hzeta]]].
    ...
    rewrite /radix2_32_block_spec.
    smt().
  inline NTRUPlus768NTTRadix2_32.M.__mul_i16
         NTRUPlus768NTTRadix2_32.M.__montgomery_reduce
         NTRUPlus768NTTRadix2_32.M.__barrett_reduce.
  ...
  + have Hcurr :
    ...
    + apply radix2_32_prefix_curr1; smt().
    have Hcurri1 :
        W64.to_uint i{hr} + block =
  ...
  have Hstep :
      (radix2_32_prefix_spec rp0 base0 (W64.to_uint i{hr} - base0) z0)
        .[base0 + (W64.to_uint i{hr} - base0) <-
          (radix2_32_pair_at rp0 base0 z0 (W64.to_uint i{hr} - base0)).`1]
        .[base0 + block + (W64.to_uint i{hr} - base0) <-
          (radix2_32_pair_at rp0 base0 z0 (W64.to_uint i{hr} - base0)).`2] =
      radix2_32_prefix_spec rp0 base0
        ((W64.to_uint i{hr} - base0) + 1) z0.
  + apply radix2_32_prefix_step; smt().
  split.
  + rewrite W64.to_uintD_small /=; smt(W64.to_uint_cmp).
  rewrite Hnext.
  rewrite -Hstep.
  rewrite /radix2_32_pair_at /radix2_32_pair /radix2_32_mul /block.
  rewrite Hget0 Haddr Hget1.
  rewrite -Hcurri Hspec_addr.
  rewrite !radix2_32_truncate_sigext_sub !radix2_32_truncate_sigext_add.
...
lemma radix2_32_block_lossless :
  islossless NTRUPlus768NTTRadix2_32.M.__radix2_32_block.
proof.
  proc.
...
lemma ntt_radix2_32_functional
...
proof.
  ...
  seq 1 : (rp = rp0).
  + auto => />.
  call (radix2_32_block_functional
          (radix2_32_block_spec
            (radix2_32_block_spec
              (radix2_32_block_spec
                (radix2_32_block_spec
                    ...
                    (radix2_32_block_spec
                      (radix2_32_block_spec
                        (radix2_32_block_spec
                          (radix2_32_block_spec
                            (radix2_32_block_spec
                              (radix2_32_block_spec rp0 0 zeta1)
                              64 zeta2)
                            128 zeta3)
                          192 zeta4)
                        256 zeta5)
  ...
  + rewrite /valid_base /double_block /nblocks.
    exists 2; smt().
  call (radix2_32_block_functional
          (radix2_32_block_spec rp0 0 zeta1) 64 zeta2).
  + rewrite /valid_base /double_block /nblocks.
    exists 1; smt().
  call (radix2_32_block_functional rp0 0 zeta1).
  + rewrite /valid_base /double_block /nblocks.
    exists 0; smt().
  auto => />.
qed.

lemma ntt_radix2_32_lossless :
  islossless
    NTRUPlus768NTTRadix2_32.M.jade_ntruplus_ntruplus768_amd64_ref_ntt_radix2_32.
proof.
  proc.
  call radix2_32_block_lossless.
  call radix2_32_block_lossless.
  call radix2_32_block_lossless.
  call radix2_32_block_lossless.
  call radix2_32_block_lossless.
