...
require import Array768 WArray1536 NTRUPlus768NTTRadix2_16.
...
op zeta1  : W16.t = W16.of_int (-455).
op zeta2  : W16.t = W16.of_int 639.
op zeta3  : W16.t = W16.of_int 502.
op zeta4  : W16.t = W16.of_int 655.
op zeta5  : W16.t = W16.of_int (-699).
op zeta6  : W16.t = W16.of_int 541.
op zeta7  : W16.t = W16.of_int 95.
op zeta8  : W16.t = W16.of_int (-1577).
op zeta9  : W16.t = W16.of_int (-1241).
op zeta10 : W16.t = W16.of_int 550.
op zeta11 : W16.t = W16.of_int (-44).
op zeta12 : W16.t = W16.of_int 39.
op zeta13 : W16.t = W16.of_int (-820).
op zeta14 : W16.t = W16.of_int (-216).
op zeta15 : W16.t = W16.of_int (-121).
op zeta16 : W16.t = W16.of_int (-757).
op zeta17 : W16.t = W16.of_int (-348).
op zeta18 : W16.t = W16.of_int 937.
op zeta19 : W16.t = W16.of_int 893.
op zeta20 : W16.t = W16.of_int 387.
op zeta21 : W16.t = W16.of_int (-603).
op zeta22 : W16.t = W16.of_int 1713.
op zeta23 : W16.t = W16.of_int (-1105).
op zeta24 : W16.t = W16.of_int 1058.
...
lemma radix2_16_modz_sub_dvd (u m d : int) :
  m %% d = 0 => (u - m) %% d = u %% d.
proof.
  move=> Hmd.
  have Hneg : (-m) %% d = 0.
...
lemma radix2_16_truncateu16_of_int (x : int) :
...
lemma radix2_16_truncate_sigext_add (x y : W16.t) :
  truncateu16 (sigextu32 x + sigextu32 y) = x + y.
proof.
  rewrite /sigextu32 /=.
  rewrite radix2_16_truncateu16_of_int W16.of_intD.
  by rewrite !radix2_16_w16_of_sintK.
qed.

lemma radix2_16_truncate_sigext_sub (x y : W16.t) :
  truncateu16 (sigextu32 x - sigextu32 y) = x - y.
proof.
  rewrite /sigextu32 /=.
  rewrite radix2_16_truncateu16_of_int W16.of_intS.
  by rewrite !radix2_16_w16_of_sintK.
qed.
                                      ...
                                      (radix2_16_block_spec
                                        (radix2_16_block_spec
                                          (radix2_16_block_spec
                                            (radix2_16_block_spec
                                              (radix2_16_block_spec
                                                (radix2_16_block_spec p 0 zeta1)
                                                32 zeta2)
                                              64 zeta3)
                                            96 zeta4)
                                          128 zeta5)
...
lemma radix2_16_mul_i16_functional (a0 b0 : W16.t) :
  hoare [NTRUPlus768NTTRadix2_16.M.__mul_i16 :
    a = a0 /\ b = b0 ==> res = mul_i16 a0 b0].
proof.
  proc; auto => />.
qed.

lemma radix2_16_montgomery_reduce_functional (a0 : W32.t) :
  hoare [NTRUPlus768NTTRadix2_16.M.__montgomery_reduce :
    a = a0 ==> res = montgomery_reduce a0].
proof.
  proc; auto => />.
qed.

lemma radix2_16_barrett_reduce_functional (a0 : W16.t) :
  hoare [NTRUPlus768NTTRadix2_16.M.__barrett_reduce :
    a = a0 ==> res = barrett_reduce a0].
proof.
  proc; auto => />.
qed.

lemma radix2_16_prefix_zero
    (p : W16.t Array768.t) (base : int) (z : W16.t) :
  0 <= base <= 768 =>
  radix2_16_prefix_spec p base 0 z = p.
proof.
  move=> Hbase.
  apply Array768.tP => j Hj.
  rewrite /radix2_16_prefix_spec Array768.initiE 1:Hj.
  smt().
qed.

lemma radix2_16_prefix_curr0
    (p : W16.t Array768.t) (base upto : int) (z : W16.t) :
  0 <= base =>
  0 <= upto < block =>
  base + block + upto < 768 =>
  (radix2_16_prefix_spec p base upto z).[base + upto] = p.[base + upto].
proof.
  move=> Hbase Hupto Hbound.
  rewrite /radix2_16_prefix_spec Array768.initiE 1:/#.
  smt().
qed.

lemma radix2_16_prefix_curr1
    (p : W16.t Array768.t) (base upto : int) (z : W16.t) :
  0 <= base =>
  0 <= upto < block =>
  base + block + upto < 768 =>
  (radix2_16_prefix_spec p base upto z).[base + block + upto] =
    p.[base + block + upto].
proof.
  move=> Hbase Hupto Hbound.
  rewrite /radix2_16_prefix_spec Array768.initiE 1:/#.
...
lemma radix2_16_prefix_step
    (p : W16.t Array768.t) (base upto : int) (z : W16.t) :
  0 <= base =>
  0 <= upto < block =>
  base + block + upto < 768 =>
  (radix2_16_prefix_spec p base upto z)
    .[base + upto <- (radix2_16_pair_at p base z upto).`1]
    .[base + block + upto <- (radix2_16_pair_at p base z upto).`2] =
  radix2_16_prefix_spec p base (upto + 1) z.
proof.
  move=> Hbase Hupto Hbound.
  apply Array768.tP => j Hj.
  rewrite /radix2_16_prefix_spec.
...
lemma radix2_16_block_functional
    (rp0 : W16.t Array768.t) (base0 : int) (z0 : W16.t) :
  valid_base base0 =>
  hoare [NTRUPlus768NTTRadix2_16.M.__radix2_16_block :
    rp = rp0 /\ base = W64.of_int base0 /\ zeta_0 = z0 ==>
    res = radix2_16_block_spec rp0 base0 z0].
proof.
  move=> Hvalid.
  have [Hbase Hbound] := valid_base_bounds base0 Hvalid.
  proc.
  seq 3 :
    (rp = radix2_16_prefix_spec rp0 base0 0 z0 /\
     i = W64.of_int base0 /\
     stop = W64.of_int base0 + W64.of_int block /\
     zeta_0 = z0).
  + auto => />.
    rewrite radix2_16_prefix_zero 1:/# /block.
    smt().
  while (base0 <= W64.to_uint i <= base0 + block /\
         stop = W64.of_int base0 + W64.of_int block /\
         zeta_0 = z0 /\
         rp = radix2_16_prefix_spec rp0 base0 (W64.to_uint i - base0) z0); last first.
  + skip => &hr [Hrp [Hi [Hstop Hzeta]]].
    ...
    rewrite /radix2_16_block_spec.
    smt().
  inline NTRUPlus768NTTRadix2_16.M.__mul_i16
         NTRUPlus768NTTRadix2_16.M.__montgomery_reduce
         NTRUPlus768NTTRadix2_16.M.__barrett_reduce.
  ...
  + have Hcurr :
    ...
    + apply radix2_16_prefix_curr1; smt().
    have Hcurri1 :
        W64.to_uint i{hr} + block =
  ...
  have Hstep :
      (radix2_16_prefix_spec rp0 base0 (W64.to_uint i{hr} - base0) z0)
        .[base0 + (W64.to_uint i{hr} - base0) <-
          (radix2_16_pair_at rp0 base0 z0 (W64.to_uint i{hr} - base0)).`1]
        .[base0 + block + (W64.to_uint i{hr} - base0) <-
          (radix2_16_pair_at rp0 base0 z0 (W64.to_uint i{hr} - base0)).`2] =
      radix2_16_prefix_spec rp0 base0
        ((W64.to_uint i{hr} - base0) + 1) z0.
  + apply radix2_16_prefix_step; smt().
  split.
  + rewrite W64.to_uintD_small /=; smt(W64.to_uint_cmp).
  rewrite Hnext.
  rewrite -Hstep.
  rewrite /radix2_16_pair_at /radix2_16_pair /radix2_16_mul /block.
  rewrite Hget0 Haddr Hget1.
  rewrite -Hcurri Hspec_addr.
  rewrite !radix2_16_truncate_sigext_sub !radix2_16_truncate_sigext_add.
...
lemma radix2_16_block_lossless :
  islossless NTRUPlus768NTTRadix2_16.M.__radix2_16_block.
proof.
  proc.
...
lemma ntt_radix2_16_functional
...
proof.
  ...
  seq 1 : (rp = rp0).
  + auto => />.
  call (radix2_16_block_functional
          (radix2_16_block_spec
            (radix2_16_block_spec
              (radix2_16_block_spec
                (radix2_16_block_spec
                                            ...
                                            (radix2_16_block_spec
                                              (radix2_16_block_spec
                                                (radix2_16_block_spec
                                                  (radix2_16_block_spec
                                                    (radix2_16_block_spec
                                                      (radix2_16_block_spec rp0 0 zeta1)
                                                      32 zeta2)
                                                    64 zeta3)
                                                  96 zeta4)
                                                128 zeta5)
  ...
  + rewrite /valid_base /double_block /nblocks.
    exists 1; smt().
  call (radix2_16_block_functional rp0 0 zeta1).
  + rewrite /valid_base /double_block /nblocks.
    exists 0; smt().
  auto => />.
qed.

lemma ntt_radix2_16_lossless :
  islossless
    NTRUPlus768NTTRadix2_16.M.jade_ntruplus_ntruplus768_amd64_ref_ntt_radix2_16.
proof.
  proc.
  call radix2_16_block_lossless.
  call radix2_16_block_lossless.
  call radix2_16_block_lossless.
  call radix2_16_block_lossless.
  call radix2_16_block_lossless.
